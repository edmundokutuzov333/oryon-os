...
function authToken(request: FastifyRequest): {
	...
	if (value?.startsWith("Bearer "))
		return { token: value.slice(7).trim(), mode: "bearer" };
...
async function currentUser(request: FastifyRequest, organizationId: string) {
...
function requireKey(request: FastifyRequest): void {
			...
			: message === "PERMISSION_DENIED"
				...
				: [
					...
					: [
						...
						? 409
						: message.startsWith("STORAGE_CONFIG_MISSING")
							? 500
							: 400;
	const code =
		status === 401
...
async function pagePermissions(
	...
	return Object.fromEntries(
		actions.map((action) => [
			action,
			can({ orgId: organizationId, ...snapshot }, resource, action).allowed,
		]),
	) as {
		read: boolean;
		create: boolean;
...
async function pageOut(
	...
	const permissionsForPage = await pagePermissions(
		organizationId,
		userId,
		page,
	);
...
async function fileOut(
	...
	const exportAllowed = can(
		{ orgId: organizationId, ...snapshot },
		resource,
		"export",
	).allowed;
	const deleteAllowed = can(
		{ orgId: organizationId, ...snapshot },
		resource,
		"delete",
	).allowed;
	...
	return FileAssetSchema.parse({
		...
		downloadUrl:
			signed && exportAllowed ? await createDownloadUrl(file.storageKey) : null,
		permissions: { read, update, export: exportAllowed, delete: deleteAllowed },
	});
}

export async function registerDocsFilesRoutesV2(
	app: FastifyInstance,
): Promise<void> {
	app.get("/v1/pages", async (request, reply) => {
		try {
			const organizationId = orgId(request);
			const user = await currentUser(request, organizationId);
			...
			for (const row of rows)
				try {
					result.push(await pageOut(organizationId, user.userId, row));
				} catch (error) {
	...
	app.post("/v1/pages", async (request, reply) => {
		try {
			...
			if (
				!can({ orgId: organizationId, ...snapshot }, resource, "create").allowed
			)
				...
				.send(
					ok(
						...
						await pageOut(
							organizationId,
							user.userId,
							await repository.createPage(organizationId, user.userId, input),
						),
					),
				);
		} catch (error) {
	...
	app.patch("/v1/pages/:id", async (request, reply) => {
		try {
			...
			const page = await repository.findPage(organizationId, id);
			if (!page) throw new Error("NOT_FOUND");
			const snapshot = await permissions.getSnapshot(
				organizationId,
				user.userId,
			...
			if (
				!can({ orgId: organizationId, ...snapshot }, resource, "update").allowed
			)
				throw new Error("PERMISSION_DENIED");
			if (
				["classification", "indexable", "publishedSlug"].some(
					(field) => field in input,
				) &&
				!can({ orgId: organizationId, ...snapshot }, resource, "manage").allowed
			)
				throw new Error("PERMISSION_DENIED");
			return reply.send(
				ok(
					...
					await pageOut(
						organizationId,
						user.userId,
						await repository.updatePage(organizationId, user.userId, id, input),
					),
				),
			);
		} catch (error) {
	...
	app.get("/v1/pages/:id/versions", async (request, reply) => {
		try {
			...
			if (
				!page ||
				!(await pagePermissions(organizationId, user.userId, page)).read
			)
				throw new Error("NOT_FOUND");
			const versions = await repository.listVersions(organizationId, page.id);
			return reply.send(
				ok(
					request,
	...
	app.post("/v1/files/complete", async (request, reply) => {
		try {
			...
			if (expectedKey !== input.storageKey)
				throw new Error("UPLOAD_KEY_MISMATCH");
				...
				.send(
					ok(
						...
						await fileOut(
							organizationId,
							user.userId,
							await repository.createFile(organizationId, user.userId, {
								...input,
	...
	app.get("/v1/search/text", async (request, reply) => {
		try {
			...
			for (const page of result.pages)
				try {
					pages.push(await pageOut(organizationId, user.userId, page));
				} catch (error) {
					if (!(error instanceof Error && error.message === "NOT_FOUND"))
						throw error;
				}
			...
			for (const file of result.files)
				try {
					files.push(await fileOut(organizationId, user.userId, file, false));
				} catch (error) {
					if (!(error instanceof Error && error.message === "NOT_FOUND"))
						throw error;
				}
			return reply.send(ok(request, { pages, files }));
		} catch (error) {
			return fail(request, reply, error);
		}
	});
