...
const sessionTtlSeconds = Number(
	process.env.ORYON_AUTH_SESSION_TTL_SECONDS ?? "2592000",
);
const accessTtlSeconds = Number(
	process.env.ORYON_AUTH_ACCESS_TTL_SECONDS ?? "900",
);
const magicLinkTtlSeconds = Number(
	process.env.ORYON_AUTH_MAGIC_LINK_TTL_SECONDS ?? "600",
);
...
function verifyJwt(token: string): {
	...
	const [encodedHeader, encodedBody, encodedSignature] = parts as [
		string,
		string,
		string,
	];
...
export async function requestMagicLink(
	...
	if (resendApiKey) {
		...
		if (!response.ok)
			throw new Error(`Unable to deliver magic link: ${response.status}`);
...
export async function verifyMagicLink(
	...
	await redisClient.set(`oryon:session-id:${sessionId}`, sessionHash, {
		EX: sessionTtlSeconds,
	});
...
export async function resolveSessionToken(token: string) {
	...
	await redisClient.set(
		key,
		JSON.stringify({ ...session, lastSeenAt: Math.floor(Date.now() / 1000) }),
		{ EX: ttl },
	);
	return {
...
export async function authenticate(
	...
	return mode === "bearer"
		? resolveBearerToken(rawToken)
		: resolveSessionToken(rawToken);
}
...
export const AUTH_COOKIE_NAME =
	process.env.ORYON_AUTH_COOKIE_NAME ?? "oryon_session";
