...
export function registerRealtime(app: FastifyInstance): Server {
  const io = new Server(app.server, {
    path: "/socket.io",
    cors: {
      origin: process.env.ORYON_WEB_URL ?? "http://localhost:3000",
      credentials: true,
    },
  });
  ...
  io.use(async (socket, next) => {
    try {
      ...
      const session = bearer ? await authenticate(bearer, "bearer") : cookieValue(socket.handshake.headers.cookie, AUTH_COOKIE_NAME);
      if (!session) return next(new Error("UNAUTHENTICATED"));
      const current = await authenticate(session, bearer ? "bearer" : "session");
