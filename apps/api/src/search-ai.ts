...
} from "@oryon/contracts/search-ai";
import {
	AiRepository,
...
const typesense =
	process.env.TYPESENSE_URL && process.env.TYPESENSE_API_KEY
...
function resourceHref(type: SearchDocumentType, id: string): string {
	if (type === "work_object")
		return `/os/work?object=${encodeURIComponent(id)}`;
	return `/os/docs?${type === "page" ? "page" : "file"}=${encodeURIComponent(id)}`;
}

function keyForMemory(
	orgId: string,
	userId: string,
...
async function readMemory(
	...
	try {
		...
		const value = await client.get(
			keyForMemory(orgId, userId, conversationKey),
		);
...
async function writeMemory(
	...
	try {
		...
		existing.push(turn);
		await client.set(key, JSON.stringify(existing.slice(-8)), {
			EX: Number(process.env.ORYON_AI_MEMORY_TTL_SECONDS ?? "2592000"),
		});
	} catch {
		return;
	}
...
async function readAndAuthorize(
	...
	return {
		source,
		snapshot,
		permissions: { read: readDecision.allowed, use_ai: aiDecision.allowed },
	};
}
...
async function hybridSearch(
	...
	if (typesense) {
		try {
			lexical = await typesense.lexical(
				query,
				orgId,
				Math.min(limit * 3, 50),
				type,
	...
	if (lexical.length === 0) {
		const fallback = await searchRepository.lexicalFallback(
			orgId,
			query,
			Math.min(limit * 3, 50),
			type,
		);
		lexical = fallback.map((hit, index) => ({
			id: `${hit.type}:${hit.id}`,
			type: hit.type,
			title: hit.title,
			snippet: hit.snippet,
	...
	if (embedding) {
		try {
			const rows = await searchRepository.semantic(
				orgId,
				embedding,
				Math.min(limit * 3, 50),
			);
			semantic = rows
				.map((row) => ({
					id: `${row.type}:${row.id}`,
					type: row.type,
					semanticScore: 1 / (1 + row.distance),
				}))
				.filter((hit) => !type || hit.type === type);
		} catch {
			semantic = [];
		}
	}
	...
	const authorized: Array<{
		...
		href: string;
		permissions: { read: boolean; use_ai: boolean };
	}> = [];
	for (const hit of fused) {
		...
		const resource = await readAndAuthorize(
			orgId,
			userId,
			typeValue,
			resourceId,
		...
		authorized.push({
			...
			snippet: lexicalHit?.snippet ?? resource.source.text.slice(0, 240),
			score: hit.score,
			matchedBy: hit.matchedBy,
			classification: resource.source.classification,
			href: resourceHref(typeValue, resource.source.id),
...
export async function registerSearchAiRoutes(
	app: FastifyInstance,
): Promise<void> {
		...
		try {
			...
			return reply
				...
				.send(
					errorEnvelope(
						request,
						status === 401 ? "UNAUTHENTICATED" : "VALIDATION_FAILED",
						status,
						message,
					),
				);
	...
	app.post("/v1/ai/search", async (request, reply) => {
		try {
			...
			for (const item of result.results) {
				const authorized = await readAndAuthorize(
					orgId,
					current.userId,
					item.type,
					item.id,
				...
				return reply
					...
					.send(
						errorEnvelope(
							...
							"NOT_FOUND",
							404,
							"No AI-readable sources found",
						),
					);

			const aiContext = await aiRepository.getContext(orgId);
			const policyRecord = aiContext.policies[0];
			const policy: AiPolicy | null = policyRecord
				? {
						allowedModels: policyRecord.allowedModels,
						fallbackModel: policyRecord.fallbackModel,
						maxClassification: policyRecord.maxClassification,
						residencyRegion: policyRecord.residencyRegion,
						monthlyCapCents: policyRecord.monthlyCapCents,
						routingRules: policyRecord.routingRules,
					}
				: null;
			const memoryPrefix =
				memory.length > 0
					? memory
							.map(
								(turn) => `Anterior: ${turn.query}\nResposta: ${turn.answer}`,
							)
							.join("\n\n")
					: "";
			const groundedQuery = memoryPrefix
				? `${memoryPrefix}\n\nNova pergunta: ${input.query}`
				: input.query;
			let answer: string;
			...
			if (input.mode === "ANSWER") {
				...
				if (config) {
					const generated = await answerWithSources(
						groundedQuery,
						sources,
						policy,
						config,
					...
					answer = sources
						...
						.map(
							(source) =>
								`${source.title}: ${source.text.slice(0, 320)} [${source.id}]`,
						)
						.join("\n\n");
				}
			} else {
			...
			if (
				...
				citationIds.some((id) => !knownIds.has(id))
			)
				return reply
					.code(502)
					.send(
						errorEnvelope(
							...
							"INTERNAL",
							502,
							"AI response did not contain resolvable citations",
						),
					);
			const citations = citationIds.map((id) => {
				...
				return {
					...
					href: resourceHref(type, resourceId),
					snippet: source?.text.slice(0, 360) ?? id,
					classification:
						result.results.find(
							(item) => item.type === type && item.id === resourceId,
						)?.classification ?? null,
				};
			});
			await writeMemory(orgId, current.userId, input.conversationKey, {
				query: input.query,
				answer: answer.slice(0, 6000),
				citations: citationIds,
				createdAt: new Date().toISOString(),
			});
			const payload = AiSearchResponseSchema.parse({
				query: input.query,
				answer,
				citations,
				model,
				retrieval: {
					...
					semantic: result.semanticCount,
					fused: result.results.length,
				},
			...
			return reply
				.code(status)
				.send(errorEnvelope(request, code, status, message));
		}
	});
		...
		try {
				...
				return reply
					...
					.send(
						errorEnvelope(
							...
							"PERMISSION_DENIED",
							403,
							"Search index management requires manage permission",
						),
					);
			if (!typesense)
				return reply
			...
			if (config) {
				for (const document of documents) {
					...
					if (embedding)
						await searchRepository.setEmbedding(
							orgId,
							document.type,
							document.id.slice(document.type.length + 1),
							embedding,
						);
				}
			}
			const payload = SearchReindexResponseSchema.parse({
				indexed,
				collection: "oryon_search_v1",
			});
			return reply.send(envelope(request, payload));
		} catch (error) {
			const message =
