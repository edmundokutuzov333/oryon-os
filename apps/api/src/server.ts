...
function requestId(request: { id: string }): string {
	return request.id;
}

function envelope<T>(request: { id: string }, data: T) {
	return { data, meta: { requestId: requestId(request), durationMs: 0 } };
}
...
function getOrg(request: { headers: Record<string, string | string[] | undefined> }): string {
	...
	if (!orgId) throw new Error("ORG_HEADER_MISSING");
	return orgId;
}
...
function getIdempotencyKey(request: { headers: Record<string, string | string[] | undefined> }): string {
	...
	if (!key) throw new Error("IDEMPOTENCY_KEY_MISSING");
	return key;
}

function getCookie(request: { headers: Record<string, string | string[] | undefined> }, name: string): string | undefined {
	...
	for (const chunk of raw.split(";")) {
		...
		if (key === name) return decodeURIComponent(chunk.slice(separator + 1).trim());
	}
	return undefined;
}
...
function bearerToken(request: { headers: Record<string, string | string[] | undefined> }): string | undefined {
	...
	if (!raw || Array.isArray(raw) || !raw.startsWith("Bearer ")) return undefined;
	return raw.slice("Bearer ".length).trim();
}

function setSessionCookie(reply: { header: (name: string, value: string) => void }, token: string): void {
	const secure = process.env.NODE_ENV === "production" ? "; Secure" : "";
	reply.header(
		"Set-Cookie",
		`${AUTH_COOKIE_NAME}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${Number(process.env.ORYON_AUTH_SESSION_TTL_SECONDS ?? "2592000")}${secure}`,
	);
}

function clearSessionCookie(reply: { header: (name: string, value: string) => void }): void {
...
app.post("/v1/auth/request-link", async (request, reply) => {
	try {
		...
		const result = await requestMagicLink(orgId, request.body);
		return reply.send(envelope(request, AuthRequestLinkResponseSchema.parse(result)));
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unable to request sign-in link";
		const code = message === "ORG_HEADER_MISSING" ? "ORG_HEADER_MISSING" : message.includes("Email provider") ? "INTERNAL" : "VALIDATION_FAILED";
		const status = code === "ORG_HEADER_MISSING" ? 400 : code === "INTERNAL" ? 500 : 400;
		return reply.code(status).send(errorEnvelope(request, code, status, message));
	}
});

app.post("/v1/auth/verify-link", async (request, reply) => {
	try {
		getIdempotencyKey(request);
		const orgId = getOrg(request);
...
app.get("/v1/auth/session", async (request, reply) => {
	try {
		...
		return reply.code(401).send(errorEnvelope(request, "UNAUTHENTICATED", 401, message));
	}
});
...
app.post("/v1/auth/logout", async (request, reply) => {
	try {
		...
		return reply.send(envelope(request, { loggedOut: true }));
	} catch (error) {
		const message = error instanceof Error ? error.message : "Unable to log out";
		const status = message === "ORG_HEADER_MISSING" ? 400 : 401;
