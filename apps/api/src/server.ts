...
function bearerToken(request: {
	...
	if (!raw || Array.isArray(raw) || !raw.startsWith("Bearer "))
		return undefined;
...
app.get("/v1/health", async (request, reply) =>
	reply.send(envelope(request, healthPayload())),
);
app.post("/v1/auth/request-link", async (request, reply) => {
	try {
		...
		return reply.send(
			envelope(request, AuthRequestLinkResponseSchema.parse(result)),
		);
	} catch (error) {
		const message =
			error instanceof Error ? error.message : "Unable to request sign-in link";
		const code =
			message === "ORG_HEADER_MISSING"
				...
				: message.includes("Email provider")
					...
					: message === "IDEMPOTENCY_KEY_MISSING"
						...
						: "VALIDATION_FAILED";
		const status =
			code === "ORG_HEADER_MISSING" ? 400 : code === "INTERNAL" ? 500 : 400;
		return reply
			.code(status)
...
app.post("/v1/auth/verify-link", async (request, reply) => {
	try {
		...
		return reply.send(
			envelope(
				request,
				AuthVerifyLinkResponseSchema.parse({
		...
		const message =
			error instanceof Error ? error.message : "Unable to verify sign-in link";
		const status =
...
app.get("/v1/auth/session", async (request, reply) => {
	try {
		...
		const currentSession = bearer
			? await authenticate(bearer, "bearer")
			: await authenticate(cookie as string, "session");
		if (currentSession.orgId !== orgId)
			return reply
				.code(401)
				.send(
					errorEnvelope(
		...
		return reply.send(
			envelope(request, await identityForSession(currentSession)),
		);
	} catch (error) {
		const message =
		...
		return reply
			.code(401)
			.send(errorEnvelope(request, "UNAUTHENTICATED", 401, message));
	}
});
	...
	try {
		...
		return reply
			...
			.send(
				errorEnvelope(
					request,
					status === 400 ? "ORG_HEADER_MISSING" : "UNAUTHENTICATED",
					status,
					message,
				),
			);
...
await registerCommunicationRoutesV2(app, {
	to: (room) => ({
