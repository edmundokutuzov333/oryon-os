...
async function channelAllowed(
	...
	const snapshot = await permissionRepository.getSnapshot(
		organizationId,
		userId,
		"channel",
		channel.id,
...
function messageResponse(
	message: Awaited<ReturnType<CommunicationRepository["createMessage"]>>,
	userId: string,
) {
...
export async function registerCommunicationRoutes(
	...
	app.get("/v1/channels", async (request, reply) => {
		try {
			...
			const current = await currentUser(request, organizationId);
			const rows = await repository.listChannels(
				organizationId,
				current.userId,
			);
			...
			const result = rows.map((channel) =>
				ChannelSchema.parse({
					...
					permissions: {
						read: true,
						post: true,
						manage: channel.createdBy === current.userId,
					},
				}),
			);
		...
		try {
			...
			io.to(`org:${organizationId}`).emit(
				...
				CommunicationEventSchema.parse({
					...
					messageId: null,
					notificationId: null,
					payload: { kind: "created", channelId: result.id },
				}),
			);
			return reply
				.code(201)
				.send(
					envelope(
						...
						ChannelSchema.parse({
							...
							memberCount: 1,
							unreadCount: 0,
							permissions: { read: true, post: true, manage: true },
						}),
					),
				);
		} catch (error) {
		...
		try {
			...
			io.to(`org:${organizationId}`).emit(
				...
				CommunicationEventSchema.parse({
					...
					notificationId: null,
					payload: {
						kind: existing ? "existing" : "created",
						channelId: result.id,
					},
				}),
			);
			return reply.send(
				envelope(
					...
					ChannelSchema.parse({
						...
						createdAt: result.createdAt.toISOString(),
						memberCount: result.kind === "DM" ? 2 : input.userIds.length + 1,
						unreadCount: 0,
						permissions: {
							read: true,
							post: true,
							manage: result.createdBy === current.userId,
						},
					}),
				),
			);
	...
	app.get("/v1/channels/:channelId/messages", async (request, reply) => {
		try {
			const organizationId = orgId(request);
			const current = await currentUser(request, organizationId);
			const channel = await repository.getMembership(
				organizationId,
			...
			const messages = result.messages.map((message) =>
				messageResponse(
					message as Awaited<
						ReturnType<CommunicationRepository["createMessage"]>
					>,
					current.userId,
				),
			);
			return reply.send(
				envelope(request, { items: messages, nextCursor: result.nextCursor }),
			);
		} catch (error) {
			return fail(request, reply, error);
		}
	...
	app.post("/v1/channels/:channelId/messages", async (request, reply) => {
		try {
			...
			io.to(`channel:${channel.channelId}`).emit("message.created", payload);
	...
	app.post("/v1/notifications/:notificationId/read", async (request, reply) => {
		try {
			...
			io.to(`user:${current.userId}`).emit("notification.created", {
	...
	app.post("/v1/channels/:channelId/read", async (request, reply) => {
		try {
			...
			return reply.send(
				envelope(request, { readAt: new Date().toISOString() }),
			);
		} catch (error) {
			return fail(request, reply, error);
		}
