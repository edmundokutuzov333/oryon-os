...
function errorResult(error: unknown): {
	...
	const message =
		error instanceof Error ? error.message : "Work experience operation failed";
	...
	if (
		["NOT_FOUND", "FILE_NOT_FOUND", "COMMENT_PARENT_NOT_FOUND"].includes(
			message,
		)
	)
...
function commentResponse(
	comment: Awaited<
		ReturnType<WorkExperienceRepository["listComments"]>
	>[number],
) {
...
function attachmentResponse(
	attachment: Awaited<
		ReturnType<WorkExperienceRepository["listAttachments"]>
	>[number],
) {
...
export async function registerWorkExperienceRoutes(
	app: FastifyInstance,
): Promise<void> {
	app.get("/v1/work-objects/:id/comments", async (request, reply) => {
		try {
			...
			return reply.send(
				envelope(
					request,
					(await experiences.listComments(orgId, object.id)).map(
						commentResponse,
					),
				),
			);
	...
	app.post("/v1/work-objects/:id/comments", async (request, reply) => {
		try {
			idempotency(request);
			const { orgId, current, object } = await authorizeObject(
				request,
				"comment",
			);
			...
			return reply
				...
				.send(
					envelope(
						request,
						commentResponse(
							await experiences.createComment(
								orgId,
								current.userId,
								object.id,
								input,
