...
import { DomainTemplateRepository, PermissionRepository } from "@oryon/db/repositories";
...
function tokenOf(request: FastifyRequest): { token: string; mode: "bearer" | "session" } | undefined {
  const bearer = headerString(request, "authorization");
  if (bearer?.startsWith("Bearer ")) return { token: bearer.slice(7).trim(), mode: "bearer" };
...
function failure(request: FastifyRequest, error: unknown) {
  const message = error instanceof Error ? error.message : "Domain template operation failed";
...
function replyForFailure(request: FastifyRequest, code: string, status: number, message: string) {
  return { status, body: errorEnvelope(request, code, status, message) };
}

function templateCollectionResource(orgId: string) {
  return { orgId, type: "object_type_def", id: orgId, workspaceId: null, projectId: null, ownerId: null, teamId: null, classification: null };
}

async function assertAccess(request: FastifyRequest, orgId: string, action: "read" | "manage") {
  ...
  const decision = can({ orgId, ...snapshot }, templateCollectionResource(orgId), action);
  if (!decision.allowed) throw new Error("PERMISSION_DENIED");
  return current;
}

export async function registerDomainTemplateRoutes(app: FastifyInstance): Promise<void> {
  app.get("/v1/domain-templates", async (request, reply) => {
    try {
      const orgId = orgIdOf(request);
      await assertAccess(request, orgId, "read");
      const result = (await templates.list(orgId)).map((item) => DomainTemplateSummarySchema.parse(item));
  ...
  app.get("/v1/domain-templates/:key", async (request, reply) => {
    try {
      ...
      return reply.send(envelope(request, DomainTemplateManifestSchema.parse(template)));
    } catch (error) {
      const result = failure(request, error);
      return reply.code(result.status).send(result.body);
    }
  ...
  app.post("/v1/domain-templates/:key/install", async (request, reply) => {
    try {
      ...
      const { key } = request.params as { key: string };
      const result = DomainTemplateInstallResponseSchema.parse(await templates.install(orgId, current.userId, DomainKeySchema.parse(key)));
  ...
  app.post("/v1/domain-templates/:key/deactivate", async (request, reply) => {
    try {
      ...
      const current = await assertAccess(request, orgId, "manage");
      const { key } = request.params as { key: string };
      const result = DomainTemplateActionResponseSchema.parse(await templates.deactivate(orgId, current.userId, DomainKeySchema.parse(key)));
      return reply.send(envelope(request, result));
    } catch (error) {
      const result = failure(request, error);
      return reply.code(result.status).send(result.body);
