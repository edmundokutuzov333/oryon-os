...
function errorResult(error: unknown) {
	const message =
		error instanceof Error ? error.message : "Docs operation failed";
	...
	if (
		[
			...
			"OWNER_NOT_FOUND",
			"FILE_NOT_FOUND",
			"STORAGE_OBJECT_SIZE_MISSING",
		].includes(message)
	)
		return { code: "NOT_FOUND", status: 404 };
	if (["PAGE_PARENT_CYCLE", "CONFLICT"].includes(message))
		return { code: "CONFLICT", status: 409 };
	if (message.startsWith("STORAGE_CONFIG_MISSING"))
...
async function pagePermissions(
	...
	return Object.fromEntries(
		actions.map((action) => [
			action,
			can({ orgId, ...snapshot }, resource, action).allowed,
		]),
	) as {
		read: boolean;
		create: boolean;
...
async function pageResponse(
	orgId: string,
	userId: string,
	page: Awaited<ReturnType<DocsFilesRepository["findPage"]>>,
) {
	...
	return PageSchema.parse({
		...page,
		position: page.position.toString(),
		contentYjsBase64: page.contentYjs
			? Buffer.from(page.contentYjs).toString("base64")
			: null,
		verifiedAt: page.verifiedAt?.toISOString() ?? null,
		nextReviewAt: page.nextReviewAt?.toISOString() ?? null,
		publishedAt: page.publishedAt?.toISOString() ?? null,
		updatedAt: page.updatedAt.toISOString(),
		createdAt: page.createdAt.toISOString(),
		permissions: permissionsForPage,
	});
}
async function fileResponse(
	orgId: string,
	userId: string,
	file: Awaited<ReturnType<DocsFilesRepository["findFile"]>>,
	...
	return FileAssetSchema.parse({
		...
		downloadUrl:
			includeDownload && exportAllowed
				? await createDownloadUrl(file.storageKey)
				: null,
		permissions: { read, update, export: exportAllowed, delete: deleteAllowed },
	});
}

export async function registerDocsFilesRoutes(
	app: FastifyInstance,
): Promise<void> {
	app.get("/v1/pages", async (request, reply) => {
		try {
			...
			for (const row of rows)
				try {
					result.push(await pageResponse(orgId, current.userId, row));
				} catch (error) {
	...
	app.post("/v1/pages", async (request, reply) => {
		try {
			...
			const orgId = orgIdOf(request);
			const current = await session(request, orgId);
			const input = PageCreateInputSchema.parse(request.body);
			const snapshot = await permissions.getSnapshot(
				orgId,
				current.userId,
				"page",
			...
			return reply
				.code(201)
				.send(
					envelope(request, await pageResponse(orgId, current.userId, page)),
				);
		} catch (error) {
			return sendError(request, reply, error);
		}
			...
			return reply.send(
				envelope(
					...
					await pageResponse(
						orgId,
						current.userId,
						await repository.findPage(orgId, id),
					),
				),
			);
		} catch (error) {
	...
	app.patch("/v1/pages/:id", async (request, reply) => {
		try {
			...
			const current = await session(request, orgId);
			const { id } = request.params as { id: string };
			const input = PageUpdateInputSchema.parse(request.body);
			const page = await repository.findPage(orgId, id);
			if (!page) throw new Error("NOT_FOUND");
			const snapshot = await permissions.getSnapshot(
				orgId,
				current.userId,
			...
			if (!can({ orgId, ...snapshot }, resource, "update").allowed)
				throw new Error("PERMISSION_DENIED");
			if (
				input.classification !== undefined ||
				input.indexable !== undefined ||
				input.publishedSlug !== undefined
			)
				if (!can({ orgId, ...snapshot }, resource, "manage").allowed)
					throw new Error("PERMISSION_DENIED");
			const updated = await repository.updatePage(
				orgId,
				current.userId,
				id,
				input,
			...
			return reply.send(
				envelope(request, await pageResponse(orgId, current.userId, updated)),
			);
		} catch (error) {
			return sendError(request, reply, error);
		}
	...
	app.get("/v1/pages/:id/versions", async (request, reply) => {
		try {
			...
			if (!page) throw new Error("NOT_FOUND");
			if (!(await pagePermissions(orgId, current.userId, page)).read)
				throw new Error("NOT_FOUND");
			const versions = await repository.listVersions(orgId, page.id);
			return reply.send(
				envelope(
					request,
					PageVersionListSchema.parse(
						versions.map((version) => ({
							...
							snapshotBytes: Buffer.from(version.snapshot).toString("base64"),
	...
	app.post("/v1/pages/:id/publish", async (request, reply) => {
		try {
			...
			const current = await session(request, orgId);
			const { id } = request.params as { id: string };
			const input = PagePublishInputSchema.parse(request.body);
			const page = await repository.findPage(orgId, id);
			if (!page) throw new Error("NOT_FOUND");
			const snapshot = await permissions.getSnapshot(
				orgId,
			...
			if (!can({ orgId, ...snapshot }, resource, "manage").allowed)
				throw new Error("PERMISSION_DENIED");
			const updated = await repository.publishPage(
				orgId,
				current.userId,
				id,
				input.publish ? input.publishedSlug : null,
			);
			return reply.send(
				envelope(request, await pageResponse(orgId, current.userId, updated)),
			);
	...
	app.delete("/v1/pages/:id", async (request, reply) => {
		try {
			...
			if (!can({ orgId, ...snapshot }, resource, "delete").allowed)
				throw new Error("PERMISSION_DENIED");
			await repository.softDeletePage(orgId, current.userId, id);
	...
	app.post("/v1/files/upload-intent", async (request, reply) => {
		try {
			...
			const orgId = orgIdOf(request);
			const current = await session(request, orgId);
			const input = FileUploadIntentInputSchema.parse(request.body);
			const snapshot = await permissions.getSnapshot(
				orgId,
				current.userId,
				"file_asset",
	...
	app.post("/v1/files/complete", async (request, reply) => {
		try {
			...
			const orgId = orgIdOf(request);
			const current = await session(request, orgId);
			const input = FileCompleteInputSchema.parse(request.body);
			const parsed = new URLSearchParams();
			parsed.set("uploadId", input.uploadId);
			const fileId = input.uploadId;
			const storageKey = storageKeyFor(
				orgId,
				current.userId,
				fileId,
			...
			if (head.mimeType && head.mimeType !== input.mimeType)
				throw new Error("FILE_MIME_MISMATCH");
			const file = await repository.createFile(orgId, current.userId, {
				...input,
				storageKey,
			});
			return reply
				...
				.send(
					envelope(
						request,
						await fileResponse(orgId, current.userId, file, false),
					),
				);
		} catch (error) {
			return sendError(request, reply, error);
	...
	app.get("/v1/files/:id", async (request, reply) => {
		try {
			...
			return reply.send(
				envelope(
					request,
					await fileResponse(orgId, current.userId, file, true),
				),
			);
		} catch (error) {
			return sendError(request, reply, error);
	...
	app.get("/v1/search/text", async (request, reply) => {
		try {
			...
			const result = await repository.search(orgId, query.q, query.limit);
			const pages = [];
			for (const page of result.pages)
				try {
					pages.push(await pageResponse(orgId, current.userId, page));
				} catch (error) {
					if (!(error instanceof Error && error.message === "NOT_FOUND"))
						throw error;
				}
			...
			for (const file of result.files)
				try {
					files.push(await fileResponse(orgId, current.userId, file, false));
				} catch (error) {
					if (!(error instanceof Error && error.message === "NOT_FOUND"))
						throw error;
				}
			return reply.send(envelope(request, { pages, files }));
		} catch (error) {
			return sendError(request, reply, error);
		}
	});
