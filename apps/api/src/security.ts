...
export function installSecurityHardening(app: FastifyInstance): void {
  app.addHook("onRequest", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (origin && allowed.length > 0 && !allowed.includes(origin)) {
      return reply.code(403).send({ error: { code: "CORS_ORIGIN_DENIED", httpStatus: 403, message: "Origin is not allowed", requestId: request.id } });
    }
    const forwarded = request.headers["x-forwarded-for"];
    const client = typeof forwarded === "string" ? forwarded.split(",")[0].trim() : request.ip;
    const now = Date.now(); const windowMs = Number(process.env.ORYON_RATE_LIMIT_WINDOW_MS ?? "60000"); const max = Number(process.env.ORYON_RATE_LIMIT_MAX ?? "300");
