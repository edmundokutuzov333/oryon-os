...
import { PermissionRepository, WorkObjectRepository } from "@oryon/db/repositories";
import { AUTH_COOKIE_NAME, authenticate } from "./auth.js";

const workObjects = new WorkObjectRepository((await import("@oryon/db")).getPrisma());
...
function errorResult(error: unknown): { code: string; status: number; message: string } {
	const message = error instanceof Error ? error.message : "Work object operation failed";
	...
	if (["NOT_FOUND", "OBJECT_TYPE_NOT_FOUND", "PARENT_NOT_FOUND", "OWNER_NOT_FOUND", "USER_NOT_FOUND", "AGENT_NOT_FOUND"].includes(message)) return { code: "NOT_FOUND", status: 404, message };
	if (error instanceof Error && error.name === "WorkObjectDomainError") return { code: "VALIDATION_FAILED", status: 400, message };
	return { code: "VALIDATION_FAILED", status: 400, message };
}

function collectionResource(orgId: string, workspaceId: string | null) {
	return { orgId, type: "work_object", id: "__collection__", workspaceId, projectId: null, ownerId: null, teamId: null, classification: null };
}
...
function permissionsFrom(snapshot: Awaited<ReturnType<PermissionRepository["getSnapshot"]>>, resource: ReturnType<typeof objectResource>) {
	const actions = ["read", "create", "update", "delete", "comment", "manage", "share", "export", "use_ai", "use_external", "view_as"] as const;
	return Object.fromEntries(actions.map((action) => [action, can({ orgId: resource.orgId, ...snapshot }, resource, action).allowed])) as WorkObjectResponse["permissions"];
}

async function toResponse(orgId: string, object: Awaited<ReturnType<WorkObjectRepository["findById"]>>, userId: string): Promise<WorkObjectResponse> {
	if (!object) throw new Error("NOT_FOUND");
	const resource = objectResource(object);
	const snapshot = await permissions.getSnapshot(orgId, userId, resource.type, resource.id, resource.classification);
	...
	return WorkObjectResponseSchema.parse({
		...
		assignments: object.assignments.map((assignment) => ({ id: assignment.id, userId: assignment.userId, agentId: assignment.agentId, role: assignment.role, allocation: assignment.allocation })),
		placements: object.placements.map((placement) => ({ id: placement.id, containerType: placement.containerType, containerId: placement.containerId, sectionId: placement.sectionId, position: placement.position.toString(), isPrimary: placement.isPrimary })),
		permissions: objectPermissions,
	});
}

export async function registerWorkObjectRoutes(app: FastifyInstance): Promise<void> {
	app.get("/v1/object-types", async (request, reply) => {
		try {
			const orgId = orgIdOf(request);
			const current = await session(request, orgId);
			const snapshot = await permissions.getSnapshot(orgId, current.userId, "object_type_def", orgId);
			const decision = can({ orgId, ...snapshot }, { orgId, type: "object_type_def", id: orgId, workspaceId: null, projectId: null, ownerId: null, teamId: null, classification: null }, "read");
			if (!decision.allowed) throw new Error("NOT_FOUND");
			const rows = await workObjects.listTypeDefs(orgId);
			const result = rows.map((row) => ObjectTypeDefSchema.parse({ id: row.id, key: row.key, name: row.name, pluralName: row.pluralName, icon: row.icon, isSystem: row.isSystem, idPrefix: row.idPrefix, schema: row.schema, statusModel: row.statusModel }));
	...
	app.post("/v1/object-types", async (request, reply) => {
		try {
			...
			const current = await session(request, orgId);
			const snapshot = await permissions.getSnapshot(orgId, current.userId, "object_type_def", orgId);
			const resource = { orgId, type: "object_type_def", id: orgId, workspaceId: null, projectId: null, ownerId: null, teamId: null, classification: null };
			if (!can({ orgId, ...snapshot }, resource, "manage").allowed) throw new Error("PERMISSION_DENIED");
			const input = WorkObjectTypeCreateInputSchema.parse(request.body);
			validateStatusModel(input.statusModel);
			const created = await workObjects.createTypeDef(orgId, current.userId, input);
	...
	app.get("/v1/work-objects", async (request, reply) => {
		try {
			...
			const collectionSnapshot = await permissions.getSnapshot(orgId, current.userId, "work_object", "__collection__");
			const collectionDecision = can({ orgId, ...collectionSnapshot }, collectionResource(orgId, query.workspaceId ?? null), "read");
	...
	app.post("/v1/work-objects", async (request, reply) => {
		try {
			...
			const snapshot = await permissions.getSnapshot(orgId, current.userId, "work_object", "__collection__");
			...
			const created = await toResponse(orgId, await workObjects.findById(orgId, id), current.userId);
			return reply.code(201).send(envelope(request, created));
		} catch (error) {
			const current = errorResult(error);
			return reply.code(current.status).send(errorEnvelope(request, current.code, current.status, current.message));
	...
	app.get("/v1/work-objects/:id", async (request, reply) => {
		try {
			...
			const current = errorResult(error);
			return reply.code(current.message === "NOT_FOUND" ? 404 : current.status).send(errorEnvelope(request, current.code, current.message === "NOT_FOUND" ? 404 : current.status, current.message));
		}
	});
	...
	app.patch("/v1/work-objects/:id", async (request, reply) => {
		try {
			...
			const object = await workObjects.findById(orgId, id);
			if (!object) throw new Error("NOT_FOUND");
			const snapshot = await permissions.getSnapshot(orgId, current.userId, "work_object", id, object.classification);
			if (!can({ orgId, ...snapshot }, objectResource(object), "update").allowed) throw new Error("PERMISSION_DENIED");
			const updated = await workObjects.update(orgId, current.userId, id, input);
			return reply.send(envelope(request, await toResponse(orgId, updated, current.userId)));
		} catch (error) {
			const current = errorResult(error);
			return reply.code(current.status).send(errorEnvelope(request, current.code, current.status, current.message));
		}
	...
	app.delete("/v1/work-objects/:id", async (request, reply) => {
		try {
			...
			if (!object) throw new Error("NOT_FOUND");
			const snapshot = await permissions.getSnapshot(orgId, current.userId, "work_object", id, object.classification);
			if (!can({ orgId, ...snapshot }, objectResource(object), "delete").allowed) throw new Error("PERMISSION_DENIED");
	...
	app.delete("/v1/work-objects/:id/assignments/:assignmentId", async (request, reply) => {
		try {
			idempotency(request);
			const orgId = orgIdOf(request);
			const current = await session(request, orgId);
			...
			if (!object) throw new Error("NOT_FOUND");
			const snapshot = await permissions.getSnapshot(orgId, current.userId, "work_object", params.id, object.classification);
			if (!can({ orgId, ...snapshot }, objectResource(object), "update").allowed) throw new Error("PERMISSION_DENIED");
			await workObjects.removeAssignment(orgId, current.userId, params.id, params.assignmentId);
			return reply.send(envelope(request, { deleted: true }));
		} catch (error) {
			const current = errorResult(error);
			return reply.code(current.status).send(errorEnvelope(request, current.code, current.status, current.message));
	...
	app.post("/v1/work-objects/:id/status", async (request, reply) => {
		try {
			...
			return reply.send(envelope(request, await toResponse(orgId, result, current.userId)));
		} catch (error) {
			const current = errorResult(error);
			return reply.code(current.status).send(errorEnvelope(request, current.code, current.status, current.message));
		}
