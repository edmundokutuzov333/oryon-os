	...
	async function submit(event: React.FormEvent<HTMLFormElement>) {
		...
		try {
			...
			const payload = (await response.json()) as {
				data?: { delivered?: boolean; debugToken?: string };
				error?: { message?: string };
			};
			if (!response.ok)
				throw new Error(
					payload.error?.message ?? "Não foi possível iniciar a autenticação.",
				);
		...
		<form className="auth-form" onSubmit={submit}>
			...
			{debugToken ? (
				<output className="debug-access">
					...
					<a
						href={`/login/verify?token=${encodeURIComponent(debugToken)}&org=${encodeURIComponent(organization)}`}
					>
						{authCopy.openAccess}
					</a>
				</output>
