...
type Evaluation = {
	...
	exposure: {
		external: { allowed: boolean; reason: string };
		ai: { allowed: boolean; reason: string };
		export: { allowed: boolean; reason: string };
		watermark: boolean;
...
export function PermissionsPanel({
	...
	const [mode, setMode] = useState<"evaluate" | "view-as" | "exposure">(
		"evaluate",
	);
	...
	async function runEvaluation(path: string, body: unknown) {
		...
		try {
			...
			if (!response.ok || !payload.data)
				throw new Error(payload.error?.message ?? copy.executionError);
	...
	async function loadExposure() {
		...
		setError(undefined);
		try {
			const response = await fetch("/api/permissions/exposure", {
				cache: "no-store",
			});
			const payload = (await response.json()) as {
				data?: Exposure;
	...
	return (
		<section className="permissions-layout" aria-label={copy.title}>
			<header className="permissions-header">
				...
				<a className="quiet-link" href="/os">
					{copy.back}
				</a>
			</header>
			<nav className="permissions-tabs" aria-label={copy.title}>
				<button
					type="button"
					className={
						mode === "evaluate" ? "permission-tab active" : "permission-tab"
					}
				...
				<button
					type="button"
					className={
						mode === "view-as" ? "permission-tab active" : "permission-tab"
					}
					onClick={() => setMode("view-as")}
				>
					{copy.viewAs}
				</button>
				<button
					type="button"
					className={
						mode === "exposure" ? "permission-tab active" : "permission-tab"
					}
					onClick={() => {
						setMode("exposure");
						void loadExposure();
			...
			{mode !== "exposure" ? (
				<div className="permission-workbench">
					...
					>
						...
						<label>
							<span>
								{copy.projectId} <small>({copy.optional})</small>
							</span>
							<input
								value={projectId}
						...
						<label>
							<span>
								{copy.ownerId} <small>({copy.optional})</small>
							</span>
							<input
								value={ownerId}
						...
						<label>
							<span>
								{copy.teamId} <small>({copy.optional})</small>
							</span>
							<input
								value={teamId}
						...
						<button type="submit" disabled={busy}>
							{busy ? copy.busy : mode === "view-as" ? copy.simulate : copy.run}
						</button>
					</form>
					<div className="permission-result" aria-live="polite">
						...
						{evaluation ? (
							<>
								<div className="permission-principal">
									<span>{copy.principal}</span>
									<strong>{evaluation.principal.email}</strong>
									<code>{evaluation.principal.id}</code>
								</div>
								<div className="permission-action-grid">
									{Object.entries(evaluation.permissions).map(
										([action, allowed]) => (
											<div
												className={
													allowed
											...
											>
												<span>{action}</span>
												<strong>{allowed ? copy.allowed : copy.denied}</strong>
											</div>
										),
									)}
								</div>
								<div className="exposure-strip">
									<div>
										...
										<strong>
											{evaluation.exposure.external.allowed
												? copy.allowed
												: copy.denied}
										</strong>
										<small>{evaluation.exposure.external.reason}</small>
									</div>
									<div>
										<span>{copy.ai}</span>
										<strong>
											{evaluation.exposure.ai.allowed
												? copy.allowed
												: copy.denied}
										</strong>
										<small>{evaluation.exposure.ai.reason}</small>
									</div>
									<div>
										<span>{copy.export}</span>
										<strong>
											{evaluation.exposure.export.allowed
												? copy.allowed
												: copy.denied}
										</strong>
										<small>{evaluation.exposure.export.reason}</small>
									</div>
									<div>
										<span>{copy.watermark}</span>
										<strong>
											{evaluation.exposure.watermark
												? copy.allowed
												: copy.denied}
										</strong>
										<small>{copy.classificationSource}</small>
									</div>
								</div>
								<div className="field-access">
									<p className="panel-kicker">{copy.fieldAccess}</p>
									{Object.entries(evaluation.exposure.fieldAccess).map(
										([field, state]) => (
											<div className="list-item" key={field}>
												<code>{field}</code>
												<span>{state}</span>
											</div>
										),
				...
				<section className="exposure-report">
					<div className="exposure-report-header">
						<div>
							<p className="panel-kicker">{copy.exposure}</p>
							<h2>{copy.title}</h2>
						</div>
						<button
							type="button"
							className="ghost-button"
							onClick={() => void loadExposure()}
							disabled={busy}
						>
							{copy.loadExposure}
						</button>
					...
					{exposure?.resources.length ? (
						<div className="item-list">
							{exposure.resources.map((row) => (
								<div
									className="list-item"
								...
								>
									...
									<span>{row.externalPrincipals}</span>
									<span>{row.fieldMasked ? copy.masked : copy.unmasked}</span>
								</div>
							))}
						</div>
					) : (
						<p className="permissions-empty">{copy.noResources}</p>
					)}
				</section>
			)}
		</section>
