...
function DraggableTask({
	...
	return (
		<button
			ref={setNodeRef}
			style={style}
			className={selected ? "oe-task oe-task-selected" : "oe-task"}
			type="button"
			onClick={onSelect}
			{...listeners}
			{...attributes}
...
function DropColumn({
	...
	return (
		<section
			ref={setNodeRef}
			className={isOver ? "oe-column oe-column-over" : "oe-column"}
...
export function WorkBoardDnd({
	...
	selectedId?: string;
	onSelect: (id: string) => void;
	onMove: (objectId: string, statusCategory: string) => Promise<void>;
}) {
	const sensors = useSensors(
		useSensor(PointerSensor, { activationConstraint: { distance: 8 } }),
	);
	const completeColumns = columnDefaults.map((base) => ({
		...base,
		items: columns.find((column) => column.key === base.key)?.items ?? [],
	}));
	async function handleDragEnd(event: DragEndEvent) {
		const destination =
			typeof event.over?.id === "string" ? event.over.id : undefined;
		if (!destination || typeof event.active.id !== "string") return;
		const source = completeColumns.find((column) =>
			column.items.some((item) => item.id === event.active.id),
		);
