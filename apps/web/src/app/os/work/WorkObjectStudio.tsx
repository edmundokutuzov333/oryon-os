...
export function WorkObjectStudio() {
	...
	const [priority, setPriority] =
		useState<(typeof priorityOptions)[number]>("NORMAL");
	...
	const selectedType = useMemo(
		() =>
			types.find(
				(type) => type.key === (editingId ? selected?.typeKey : typeKey),
			),
		[editingId, selected, typeKey, types],
	);
	...
	async function request(path: string, init?: RequestInit): Promise<unknown> {
		...
		if (!response.ok || payload.data === undefined)
			throw new Error(payload.error?.message ?? workCopy.error);
		return payload.data;
	}
	...
	async function reload() {
		...
		try {
			...
			setTypes(nextTypes);
			setWorkspaceId(
				(current) => current || nextIdentity.workspaces[0]?.id || "",
			);
			setTypeKey((current) => current || nextTypes[0]?.key || "");
			const workspace = nextIdentity.workspaces[0];
			const data = await request(
				`/api/work-objects?limit=100${workspace ? `&workspaceId=${encodeURIComponent(workspace.id)}` : ""}`,
			);
			...
			setError(
				requestError instanceof Error ? requestError.message : workCopy.error,
			);
		} finally {
			setBusy(false);
		}
	...
	async function saveObject() {
		...
		try {
			...
			const data = editingId
				? await request("/api/work-object", {
						method: "PATCH",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({ id: editingId, ...body }),
					})
				: await request("/api/work-objects", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
	...
	async function changeStatus(value: string) {
		...
		setError(undefined);
		try {
			const data = await request("/api/work-object", {
				method: "PATCH",
				headers: { "Content-Type": "application/json" },
	...
	async function createType() {
		...
		try {
			...
			await request("/api/object-types", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					...newType,
					schema: {
						fields: fieldDrafts.filter((field) => field.key && field.label),
					},
					statusModel: { initial: states[0].key, states, transitions },
				}),
			});
	...
	if (busy)
		return (
			<section className="work-studio">
				<div className="work-empty">{workCopy.loading}</div>
			</section>
		);
		...
		<section className="work-studio" aria-label={workCopy.title}>
			...
			<div className="work-tabs" role="tablist" aria-label={workCopy.title}>
				<button
					...
					className={mode === "objects" ? "work-tab active" : "work-tab"}
				...
				<button
					...
					className={mode === "types" ? "work-tab active" : "work-tab"}
			...
			{message ? (
				<p className="work-success" role="status">
					{message}
				</p>
			) : null}
			{mode === "objects" ? (
				<div className="work-grid">
					<section className="work-list-panel" aria-label={workCopy.objects}>
						<div className="work-panel-head">
							<div>
								<p className="panel-kicker">{workCopy.engineStatus}</p>
								<strong>{workCopy.engineDescription}</strong>
							</div>
							<span className="work-count">{objects.length}</span>
						</div>
						{objects.length ? (
							<div className="work-object-list">
								{objects.map((object) => (
									<button
										type="button"
										key={object.id}
										className={
											object.id === selectedId
												? "work-object-row active"
												: "work-object-row"
										}
										onClick={() => editObject(object)}
									>
										<div>
											<span>{object.typeKey}</span>
											<strong>{object.title}</strong>
										</div>
										<div className="work-row-meta">
											<code>{object.humanId}</code>
							...
							<div className="work-empty">{workCopy.noObjects}</div>
						)}
					</section>
					<section
						className="work-editor"
						aria-label={selected ? workCopy.selectedObject : workCopy.newObject}
					>
						<div className="work-editor-head">
							<div>
								<p className="panel-kicker">
									{editingId ? workCopy.selectedObject : workCopy.newObject}
								</p>
								<h2>{editingId ? selected?.humanId : workCopy.newObject}</h2>
							</div>
							{selected?.permissions.delete ? (
								<button
									type="button"
									className="work-danger-button"
									onClick={() => void deleteObject()}
									disabled={saving}
								>
									{saving ? workCopy.deleting : workCopy.delete}
								</button>
							) : null}
						</div>
						<div className="work-form-grid">
							<label>
								<span>{workCopy.type}</span>
								<select
									value={typeKey}
									onChange={(event) => {
							...
							<label className="span-2">
								<span>{workCopy.description}</span>
								<textarea
									value={description}
							...
							{editingId && selected ? (
								<label>
									<span>{workCopy.status}</span>
									<select
										value={selected.status}
										onChange={(event) => void changeStatus(event.target.value)}
										disabled={saving || !selected.permissions.update}
									>
										{selectedType?.statusModel.states.map((state) => (
						...
						{selectedType ? (
							<div className="work-custom-fields">
								<p className="panel-kicker">{workCopy.customFields}</p>
								{selectedType.schema.fields.length ? (
									selectedType.schema.fields.map((field) => (
										<label key={field.key}>
											<span>
												{field.label}
												{field.required ? workCopy.requiredMark : ""}
											</span>
											{field.type === "BOOLEAN" ? (
												<input
													type="checkbox"
												...
												>
													<option value="">{workCopy.optional}</option>
												...
												<input
													value={
														Array.isArray(customFields[field.key])
															? (customFields[field.key] as unknown[])
																	.map(String)
																	.join(", ")
															: ""
													}
												...
												<input
													...
													value={
														field.type === "DATE" &&
														typeof customFields[field.key] === "string"
															? String(customFields[field.key]).slice(0, 16)
									...
									<p className="work-muted">{workCopy.noTypes}</p>
								)}
							</div>
						) : null}
						<div className="work-editor-footer">
							{selected ? (
								<div className="work-object-facts">
									<span>
										<code>{selected.humanId}</code>
									</span>
									<span>{selected.statusCategory}</span>
									<span>{selected.progress}%</span>
									<span>{selected.ownerId ?? workCopy.emptyValue}</span>
								</div>
							) : null}
							<button
								type="button"
								...
								disabled={saving || !title.trim() || !typeKey}
							>
								{saving
									? editingId
										? workCopy.updating
				...
				<div className="type-builder-grid">
					<section className="type-builder-form">
						<div>
							<p className="panel-kicker">{workCopy.newType}</p>
							<h2>{workCopy.types}</h2>
						</div>
						<label>
							<span>{workCopy.key}</span>
							<input
								value={newType.key}
								onChange={(event) =>
									setNewType({ ...newType, key: event.target.value })
								}
							/>
						</label>
						<label>
							<span>{workCopy.name}</span>
							<input
								value={newType.name}
								onChange={(event) =>
						...
						<label>
							<span>{workCopy.pluralName}</span>
							<input
								value={newType.pluralName}
								onChange={(event) =>
									setNewType({ ...newType, pluralName: event.target.value })
								}
							/>
						</label>
						<label>
							<span>{workCopy.idPrefix}</span>
							<input
								value={newType.idPrefix}
								onChange={(event) =>
									setNewType({
										...newType,
										idPrefix: event.target.value.toUpperCase(),
									})
								}
							/>
						</label>
						<label>
							<span>{workCopy.statuses}</span>
							<textarea
								rows={7}
								value={statusLines}
								onChange={(event) => setStatusLines(event.target.value)}
							/>
						</label>
						<label>
							<span>{workCopy.transitions}</span>
							<textarea
								rows={7}
								value={transitionLines}
								onChange={(event) => setTransitionLines(event.target.value)}
							/>
						</label>
						<div className="type-field-list">
							<div className="work-panel-head">
								<div>
									<p className="panel-kicker">{workCopy.fields}</p>
								</div>
								<button
									type="button"
									className="ghost-button"
									onClick={() =>
										setFieldDrafts((current) => [...current, { ...emptyField }])
									}
								>
									{workCopy.addField}
								</button>
							...
							{fieldDrafts.map((field, index) => (
								<div className="type-field-row" key={`${index}-${field.key}`}>
									<input
										aria-label={workCopy.fieldKey}
										value={field.key}
										onChange={(event) =>
									...
									/>
									<input
										aria-label={workCopy.fieldLabel}
										value={field.label}
										onChange={(event) =>
											setFieldDrafts((current) =>
												current.map((item, itemIndex) =>
						...
						>
							{saving ? workCopy.creatingType : workCopy.createType}
						</button>
					</section>
					<section className="type-catalog">
						<p className="panel-kicker">{workCopy.types}</p>
						<div className="type-catalog-list">
							{types.map((type) => (
								<article className="type-catalog-card" key={type.id}>
									<div>
										<code>{type.idPrefix}</code>
										<h3>{type.name}</h3>
										<p>{type.pluralName}</p>
									</div>
									<div className="type-card-meta">
										<span>
											...
											{workCopy.fields.toLowerCase()}
										</span>
										<span>
											{type.statusModel.states.length}{" "}
											{workCopy.statuses.toLowerCase()}
										</span>
									</div>
								</article>
							))}
