...
import type { DomainTemplateManifest, DomainTemplateSummary } from "@oryon/contracts/domain-templates";
import "./templates.css";

const COPY = {
  eyebrow: "OryonOS / Domain Templates",
  title: "Equipas prontas, motor único.",
  description: "Active um domínio operacional sem criar uma aplicação paralela. CRM, Support e Product / Engineering usam WorkObject, estados, campos, relações e permissões do mesmo núcleo.",
  all: "Todos",
  installed: "Activos",
  ...
  conflict: "Existe uma definição com uma chave usada pelo template. Os dados existentes foram preservados.",
  active: "Activo",
  inactive: "Inactivo",
...
export function DomainTemplatesStudio() {
  ...
  const [manifests, setManifests] = useState<Map<string, DomainTemplateManifest>>(new Map());
  ...
  const load = async () => {
    setError(null);
    const response = await fetch("/api/domain-templates", { cache: "no-store" });
    if (!response.ok) {
      setError(response.status === 403 ? COPY.noAccess : COPY.empty);
      return;
    }
    const payload = (await response.json()) as { data: DomainTemplateSummary[] };
    setTemplates(payload.data);
    const next = new Map<string, DomainTemplateManifest>();
    await Promise.all(payload.data.map(async (item) => {
      const detailResponse = await fetch(`/api/domain-templates/${item.key}`, { cache: "no-store" });
      if (detailResponse.ok) {
        const detail = (await detailResponse.json()) as { data: DomainTemplateManifest };
  ...
  const act = async (key: string, action: "install" | "deactivate") => {
    ...
    setNotice(null);
    try {
      const response = await fetch(`/api/domain-templates/${key}/${action}`, { method: "POST", headers: { "Idempotency-Key": crypto.randomUUID(), "Content-Type": "application/json" }, body: "{}" });
      const payload = (await response.json()) as { error?: { message?: string } };
      if (!response.ok) throw new Error(payload.error?.message ?? COPY.empty);
      await load();
      setNotice(COPY.refreshed);
    } catch (cause) {
      setError(cause instanceof Error && cause.message.includes("CONFLICT") ? COPY.conflict : cause instanceof Error ? cause.message : COPY.empty);
    } finally {
      setBusyKey(null);
    }
  };
  ...
  return (
    <main className="domain-template-page">
      ...
      {notice ? <Card className="domain-template-feedback"><p>{notice}</p></Card> : null}
      {!templates.length && !error ? <Card className="domain-template-feedback"><p>{COPY.loading}</p></Card> : null}
      {templates.length > 0 && visible.length === 0 ? <Card className="domain-template-feedback"><p>{COPY.empty}</p></Card> : null}
        ...
        {visible.map((template) => (
          <Card key={template.key} className="domain-template-card">
            ...
            <div>
              ...
              <p className="domain-template-summary">{template.shortDescription}</p>
            </div>
            <div className="domain-template-metrics">
              <span><strong>{template.objectTypeCount}</strong> {COPY.objectTypes}</span>
              <span><strong>{template.journeyCount}</strong> {COPY.journeys}</span>
            </div>
        ...
        <section className="domain-template-detail" aria-label={selected.name}>
          <Card className="domain-template-detail-card">
            ...
            <div className="domain-template-detail-body">
              ...
              <div className="domain-template-detail-section"><h3>{COPY.relations}</h3><div className="domain-template-list">{selected.relations.map((relation) => <div key={`${relation.fromTypeKey}-${relation.toTypeKey}-${relation.relation}`} className="domain-template-list-row"><code>{relation.fromTypeKey}</code><span>{relation.label}</span><code>{relation.toTypeKey}</code></div>)}</div></div>
              <div className="domain-template-detail-section"><h3>{COPY.journeysLabel}</h3><div className="domain-template-list">{selected.journeys.map((journey) => <div key={journey.key} className="domain-template-journey"><div><strong>{journey.name}</strong><p>{journey.description}</p></div><span>{journey.steps.map((step) => step.label).join(" → ")}</span></div>)}</div></div>
            </div>
          </Card>
        </section>
      ) : null}
