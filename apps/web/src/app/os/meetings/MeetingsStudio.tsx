...
function LiveMeeting({ meeting, onClose, onEnded }: { meeting: Meeting; onClose: () => void; onEnded: () => void }) {
	...
	async function toggleMic() { const room = roomRef.current; if (!room) return; const enabled = await room.localParticipant.setMicrophoneEnabled(!mic); setMic(enabled); refreshParticipants(); }
	async function toggleCamera() { const room = roomRef.current; if (!room) return; const enabled = await room.localParticipant.setCameraEnabled(!camera); setCamera(enabled); refreshParticipants(); }
	async function toggleScreen() { const room = roomRef.current; if (!room) return; const enabled = await room.localParticipant.setScreenShareEnabled(!screen); setScreen(enabled); refreshParticipants(); }
	...
	return <div className="oryon-meeting-call">
		<header className="oryon-meeting-call-header"><div><span className="oryon-eyebrow">{state === "connected" ? COPY.connected : COPY.waiting}</span><h2>{meeting.title}</h2></div><div className="oryon-meeting-call-meta"><StatusChip label={`${participants.length} ${COPY.participants.toLocaleLowerCase("pt-PT")}`} tone="neutral" /><Button variant="ghost" size="sm" onClick={onClose}>{COPY.close}</Button></div></header>
