...
function nodePoint(index: number, total: number): Point {
	...
	const angle =
		((index - 1) / Math.max(total - 1, 1)) * Math.PI * 2 - Math.PI / 2;
	const radiusX = 315;
	const radiusY = 205;
	return {
...
export function GraphStudio() {
	...
	const graphObjects = useMemo(
		() => graph?.nodes.filter((node) => node.type === "work_object") ?? [],
		[graph],
	);
	...
	async function request(path: string, init?: RequestInit): Promise<unknown> {
		...
		if (!response.ok || payload.data === undefined) {
			...
			if (backendMessage === "PERMISSION_DENIED")
				throw new Error(graphCopy.permission);
	...
	async function loadObjects(): Promise<void> {
		...
		setTargetId(
			(current) =>
				current ||
				parsed.find((object) => object.id !== (parsed[0]?.id ?? ""))?.id ||
				"",
		);
	}
	...
	async function loadGraph(nextRootId = rootId): Promise<void> {
		...
		try {
			const effectiveDepth =
				mode === "customer360" ? Math.max(depth, 3) : depth;
			...
			setError(
				requestError instanceof Error ? requestError.message : graphCopy.error,
			);
		} finally {
			setBusy(false);
		}
	...
	async function connect(): Promise<void> {
		...
		try {
			const input = GraphEdgeCreateInputSchema.parse({
				from: { type: "work_object", id: rootId },
				to: { type: "work_object", id: targetId },
				relation: newRelation,
				metadata: {},
			});
			await request("/api/edges", {
				method: "POST",
		...
		() =>
			new Map(
				graphObjects.map((node, index) => [
					node.id,
					nodePoint(index, graphObjects.length),
				]),
			),
		[graphObjects],
	);

	if (busy && objects.length === 0)
		return (
		...
		<section className="graph-studio" aria-label={graphCopy.title}>
			<header className="graph-header">
				<div>
					<p className="graph-kicker">
						{graphCopy.eyebrow.toLocaleLowerCase("pt-PT")}
					</p>
			...
			<div className="graph-tabs" role="tablist" aria-label={graphCopy.title}>
				<button
					...
					className={mode === "network" ? "graph-tab active" : "graph-tab"}
				...
				<button
					type="button"
					role="tab"
					aria-selected={mode === "customer360"}
					className={mode === "customer360" ? "graph-tab active" : "graph-tab"}
					onClick={() => setMode("customer360")}
				>
					{graphCopy.customer360}
				</button>
			</div>

			<div className="graph-controls">
				<label>
					<span>{graphCopy.object}</span>
					<select
						value={rootId}
					...
					>
						{objects.map((object) => (
							<option key={object.id} value={object.id}>
								{object.humanId} · {object.title}
							</option>
						))}
					</select>
				</label>
				<label>
					<span>{graphCopy.direction}</span>
					<select
						value={direction}
						onChange={(event) =>
							setDirection(event.target.value as "out" | "in" | "both")
						}
					>
						<option value="both">{graphCopy.both}</option>
						<option value="out">{graphCopy.outgoing}</option>
						<option value="in">{graphCopy.incoming}</option>
					</select>
				</label>
				<label>
					<span>{graphCopy.depth}</span>
					<select
						value={depth}
				...
				<label>
					<span>{graphCopy.relations}</span>
					<select
						value={relation}
						onChange={(event) =>
							setRelation(event.target.value as GraphRelation | "*")
						}
					>
						<option value="*">{graphCopy.allRelations}</option>
						{graphRelations.map((item) => (
							<option key={item} value={item}>
								{labelForRelation(item)}
							</option>
						))}
					</select>
				</label>
			...
			{message ? (
				<p className="graph-success" role="status" aria-live="polite">
					{message}
				</p>
			) : null}
			...
			<div className="graph-layout">
				<section
					...
					aria-label={graphCopy.network}
					aria-busy={busy}
				>
					<div className="graph-canvas-meta">
						<span>
							{graph?.meta.visibleNodeCount ?? 0} {graphCopy.nodeCount}
						</span>
						<span>
							{graph?.edges.length ?? 0} {graphCopy.edgeCount}
						</span>
						{graph?.meta.truncated ? <span>{graphCopy.truncated}</span> : null}
						{busy ? <span>{graphCopy.loading}</span> : null}
					</div>
					{graph && graph.nodes.length > 0 ? (
						...
						>
							{graph.edges.map((edge) => {
								...
								return (
									<g key={edge.id}>
										...
										>
											{labelForRelation(edge.relation)}
										</text>
									</g>
								);
							})}
							{graphObjects.map((node, index) => {
								...
								if (!point || !("humanId" in node) || !("title" in node))
									return null;
								...
								return (
									<g
										...
										role="button"
										tabIndex={0}
										aria-label={`${node.humanId} ${node.title}`}
										onKeyDown={(event) => {
						...
						<div className="graph-empty">{graphCopy.noGraph}</div>
					)}
				</section>
				...
				<aside className="graph-side-panel">
					<section className="graph-card">
						...
						<div className="graph-facts">
							<span>{rootObject?.humanId ?? ""}</span>
							<span>{rootObject?.status ?? ""}</span>
							<span>{rootObject?.priority ?? ""}</span>
						</div>
						<dl>
							<div>
								<dt>{graphCopy.owner}</dt>
								<dd>{displayOwner(rootObject)}</dd>
							</div>
							<div>
								<dt>{graphCopy.workspace}</dt>
								<dd>{rootObject?.workspaceId ?? ""}</dd>
							</div>
							<div>
								<dt>{graphCopy.relationshipCount}</dt>
					...
					<section className="graph-card">
						<p className="graph-kicker">{graphCopy.connect}</p>
						<h2>{graphCopy.createRelation}</h2>
						<label>
							<span>{graphCopy.to}</span>
							<select
								value={targetId}
								onChange={(event) => setTargetId(event.target.value)}
								disabled={!objects.some((object) => object.id !== rootId)}
							>
								{objects
									.filter((object) => object.id !== rootId)
									.map((object) => (
										<option key={object.id} value={object.id}>
											{object.humanId} · {object.title}
										</option>
						...
						<label>
							<span>{graphCopy.relation}</span>
							<select
								value={newRelation}
								onChange={(event) =>
									setNewRelation(event.target.value as GraphRelation)
								}
							>
								{graphRelations.map((item) => (
									<option key={item} value={item}>
						...
						<button
							...
							disabled={saving || !targetId || !rootId}
						>
							{saving ? graphCopy.newRelation : graphCopy.connect}
						</button>
					</section>

					<section className="graph-card graph-timeline">
						<p className="graph-kicker">
							{mode === "customer360"
								? graphCopy.customerContext
								: graphCopy.timeline}
						</p>
						{graph?.timeline.length ? (
							<div className="graph-timeline-list">
								{graph.timeline.slice(0, 18).map((event) => (
									<button
										type="button"
										key={event.id}
										className="graph-timeline-row"
										onClick={() => setRootId(event.node.id)}
									>
							...
							<div className="graph-empty">{graphCopy.noTimeline}</div>
						)}
					</section>
				</aside>
			</div>
