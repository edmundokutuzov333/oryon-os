...
export async function PATCH(request: Request) {
	...
	if (!body.id) return NextResponse.json({ error: { message: "VALIDATION_FAILED" } }, { status: 400 });
	const { id, ...input } = body;
	const response = await fetch(`${apiUrl}/v1/work-objects/${encodeURIComponent(id)}`, { method: "PATCH", headers: { ...headers, "Idempotency-Key": randomUUID() }, body: JSON.stringify(input), cache: "no-store" });
...
export async function DELETE(request: Request) {
	...
	if (!body.id) return NextResponse.json({ error: { message: "VALIDATION_FAILED" } }, { status: 400 });
	const response = await fetch(`${apiUrl}/v1/work-objects/${encodeURIComponent(body.id)}`, { method: "DELETE", headers: { ...headers, "Idempotency-Key": randomUUID() }, cache: "no-store" });
	return NextResponse.json(await response.json(), { status: response.status });
}
