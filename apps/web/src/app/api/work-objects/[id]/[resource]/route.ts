...
export async function GET(_request: Request, context: { params: Promise<{ id: string; resource: string }> }) {
  ...
  if (!["comments", "attachments", "history"].includes(resource)) return NextResponse.json({ error: { message: "NOT_FOUND" } }, { status: 404 });
  const response = await fetch(`${apiUrl}/v1/work-objects/${encodeURIComponent(id)}/${resource}`, { headers, cache: "no-store" });
...
export async function POST(request: Request, context: { params: Promise<{ id: string; resource: string }> }) {
  ...
  if (!headers) return NextResponse.json({ error: { message: "UNAUTHENTICATED" } }, { status: 401 });
  const { id, resource } = await context.params;
  if (!["comments", "attachments"].includes(resource)) return NextResponse.json({ error: { message: "NOT_FOUND" } }, { status: 404 });
  const body = await request.text();
  const response = await fetch(`${apiUrl}/v1/work-objects/${encodeURIComponent(id)}/${resource}`, { method: "POST", headers: { ...headers, "Idempotency-Key": randomUUID() }, body, cache: "no-store" });
  return NextResponse.json(await response.json(), { status: response.status });
}
