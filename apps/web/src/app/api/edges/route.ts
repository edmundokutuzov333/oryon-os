...
async function authHeaders(): Promise<HeadersInit | null> {
	...
	return { "Content-Type": "application/json", "X-Oryon-Org": organization, Cookie: `${cookieName}=${encodeURIComponent(session)}`, "Idempotency-Key": randomUUID() };
}

export async function POST(request: Request): Promise<Response> {
	const headers = await authHeaders();
	if (!headers) return NextResponse.json({ error: { message: "UNAUTHENTICATED" } }, { status: 401 });
	const response = await fetch(`${apiUrl}/v1/edges`, { method: "POST", headers, body: JSON.stringify(await request.json()), cache: "no-store" });
