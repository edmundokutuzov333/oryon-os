...
async function forward(
	...
	const url = new URL(
		`${apiUrl}/v1/${segments.map(encodeURIComponent).join("/")}`,
	);
...
export async function GET(
	...
	return forward(request, (await context.params).segments, "GET");
}
export async function POST(
	request: Request,
	context: { params: Promise<{ segments: string[] }> },
) {
	return forward(request, (await context.params).segments, "POST");
}
export async function PATCH(
	request: Request,
	context: { params: Promise<{ segments: string[] }> },
) {
	return forward(request, (await context.params).segments, "PATCH");
}
export async function DELETE(
	request: Request,
	context: { params: Promise<{ segments: string[] }> },
) {
	return forward(request, (await context.params).segments, "DELETE");
}
