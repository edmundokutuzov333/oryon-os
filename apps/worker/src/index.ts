...
const triggerQueue = new (await import("bullmq")).Queue(
	"oryon.workflow-triggers",
	{ connection: redisConnection },
);
const agentTriggerQueue = new (await import("bullmq")).Queue(
	"oryon.agent-triggers",
	{ connection: redisConnection },
);
let stopped = false;
...
async function pollRunnable(): Promise<void> {
	for (const orgId of await automation.listOrganizations()) {
		for (const run of await agents.listRunnable(orgId))
			await enqueueAgentRun(orgId, run.id);
		for (const run of await automation.listRunnable(orgId))
			await enqueueWorkflowRun(orgId, run.id);
	}
}
...
async function pollEvents(): Promise<void> {
	for (const orgId of await automation.listOrganizations()) {
		...
		for (const event of events) {
			for (const hook of await webhookDeliveries.listWebhookTargets(
				orgId,
				event.name,
			)) {
				const delivery = await webhookDeliveries.createDelivery(
					orgId,
					hook.id,
					event.id,
				);
			...
			for (const workflow of workflows) {
				...
				if (trigger.kind !== "EVENT" || trigger.eventName !== event.name)
					continue;
				const existing = await automation.findRunByTriggerEvent(
					orgId,
					workflow.id,
					event.id,
				);
				if (!existing) {
					...
					await enqueueWorkflowRun(orgId, created.id);
				}
			}
			await webhookDeliveries.markEventPublished(orgId, event.id);
		}
		for (const deliveryId of await webhookDeliveries.listPendingDeliveries(
			orgId,
			200,
		))
			await enqueueWebhookDelivery(orgId, deliveryId);
	}
}
async function configureSchedules(): Promise<void> {
	for (const orgId of await automation.listOrganizations()) {
		for (const workflow of await automation.list(orgId)) {
			...
			const trigger = WorkflowTriggerSchema.safeParse(
				workflow.triggerJson,
			).data;
			if (trigger?.kind === "SCHEDULE")
				await triggerQueue.upsertJobScheduler(
					`workflow-schedule:${orgId}:${workflow.id}`,
					{ pattern: trigger.cron },
					{
						name: "workflow.schedule.trigger",
						data: { orgId, workflowId: workflow.id },
						opts: { removeOnComplete: 1000, removeOnFail: 1000 },
					},
				);
		}
		for (const agent of await agents.list(orgId))
			if (agent.status === "ACTIVE" && agent.schedule)
				await agentTriggerQueue.upsertJobScheduler(
					`agent-schedule:${orgId}:${agent.id}`,
					{ pattern: agent.schedule },
					{
						name: "agent.schedule.trigger",
						data: { orgId, agentId: agent.id },
						opts: { removeOnComplete: 1000, removeOnFail: 1000 },
					},
				);
	}
...
async function startTemporal(): Promise<{
	...
	const worker = await TemporalWorker.create({
		...
		taskQueue: process.env.TEMPORAL_TASK_QUEUE ?? "oryon-agents",
...
async function main(): Promise<void> {
	...
	const workflowTriggerWorker = new BullWorker(
		"oryon.workflow-triggers",
		async (job) => {
			...
			if (temporal)
				await temporal.client.workflow.start(runOryonWorkflow, {
					taskQueue: process.env.TEMPORAL_TASK_QUEUE ?? "oryon-agents",
					workflowId: `workflow:${created.id}`,
					args: [data.orgId, created.id],
				});
			else await enqueueWorkflowRun(data.orgId, created.id);
		},
		{ connection: redisConnection, concurrency: 4 },
	);
	const agentTriggerWorker = new BullWorker(
		"oryon.agent-triggers",
		async (job) => {
	...
	while (!stopped) {
		...
		await new Promise((resolve) =>
			setTimeout(resolve, Number(process.env.ORYON_AGENT_POLL_MS ?? "2000")),
		);
	}
}
