...
export async function executeWorkflowRun(orgId: string, workflowRunId: string): Promise<void> {
	...
	for (const step of definition) {
		...
		if (existing?.status === "WAITING" && !run.waitingOnApprovalId && step.type === "APPROVAL") { existing.status = "SUCCEEDED"; existing.finishedAt = new Date().toISOString(); continue; }
		const startedAt = new Date().toISOString(); const current: StepLog = existing?.status === "WAITING" ? existing : { id: step.id, status: "RUNNING", startedAt, finishedAt: null }; current.status = "RUNNING"; current.startedAt = current.startedAt ?? startedAt; current.finishedAt = null; current.error = undefined; log.splice(0, log.length, ...log.filter((entry) => entry.id !== step.id), current); await automation.updateRun(orgId, workflowRunId, { state: "RUNNING", stepLog: log }, actorId);
		try {
			...
			if (step.type === "CONDITION") { if (!step.condition) throw new Error("VALIDATION_FAILED"); const condition = WorkflowConditionSchema.parse(step.condition); const matched = evaluateWorkflowCondition(condition, input); current.status = matched ? "SUCCEEDED" : "SKIPPED"; output = { matched }; }
				...
				if (run.waitingOnApprovalId && run.waitingOnApprovalId === `${workflowRunId}:${step.id}`) return;
			...
			current.output = output; current.finishedAt = new Date().toISOString(); await automation.updateRun(orgId, workflowRunId, { stepLog: log, waitingOnApprovalId: null }, actorId);
