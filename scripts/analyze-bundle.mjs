...
if (!roots.some(existsSync)) {
	console.log(
		"Bundle analysis: build output not present, CI build will populate it before this gate.",
	);
	process.exit(0);
}
const files = [];
	...
	if (existsSync(root))
		for (const name of readdirSync(root)) {
			...
			if (statSync(path).isFile() && path.endsWith(".js")) {
				...
				files.push({ path, gzip: gzipSync(raw).length });
			}
		}
...
if (violations.length)
	throw new Error(
		`Bundle budget exceeded by ${violations.length} chunk(s); max ${max} bytes gzip`,
	);
