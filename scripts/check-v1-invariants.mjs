...
for (const primitive of ["model WorkObject {", "model ObjectTypeDef {", "model ObjectPlacement {", "model Assignment {", "model StatusTransition {"]) {
	if (!schema.includes(primitive)) {
		console.error(`V1 invariant failed: work object primitive missing: ${primitive}`);
		process.exit(1);
	}
}
if (!schema.includes("model Edge {") || !schema.includes("enum EdgeRelation {") || !schema.includes("@@unique([fromType, fromId, toType, toId, relation])")) {
	console.error("V1 invariant failed: canonical work graph primitives missing");
	process.exit(1);
}
for (const relation of ["BLOCKS", "PARENT_OF", "DERIVED_FROM", "CONVERTED_TO", "RELATES_TO"]) {
	if (!schema.includes(`  ${relation}`)) {
		console.error(`V1 invariant failed: required graph relation missing: ${relation}`);
		process.exit(1);
	}
}
...
for (const required of ["export function graphNodeKey", "export function cycleWouldExist", "export function assertNoProtectedCycle", "export function validateGraphSelfEdge"]) {
	if (!graphEngine.includes(required)) {
		console.error(`V1 invariant failed: work graph engine entrypoint missing: ${required}`);
		process.exit(1);
	}
}
const graphContracts = await readFile("packages/contracts/src/graph.schema.ts", "utf8");
for (const required of ["export const GraphEdgeCreateInputSchema", "export const GraphTraverseQuerySchema", "export const GraphTraverseResponseSchema"]) {
	if (!graphContracts.includes(required)) {
		console.error(`V1 invariant failed: work graph contract missing: ${required}`);
		process.exit(1);
	}
}
