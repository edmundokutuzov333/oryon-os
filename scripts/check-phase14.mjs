...
for (const key of ["crm", "support", "product_engineering"]) {
	if (!manifests.includes(`key: "${key}"`))
		throw new Error(`Missing domain template: ${key}`);
}

const schema = readFileSync("packages/db/prisma/schema.prisma", "utf8");
if (schema.includes("model DomainTemplate"))
	throw new Error(
		"Phase 14 must not introduce a parallel DomainTemplate persistence model",
	);
...
for (const marker of [
	...
	if (!repo.includes(marker))
		throw new Error(
			`Domain template repository missing required invariant: ${marker}`,
		);
}
...
for (const route of ["/v1/domain-templates", "/install", "/deactivate"]) {
	if (!api.includes(route))
		throw new Error(`Missing API route marker: ${route}`);
}

const shell = readFileSync("apps/web/src/app/os/OryonAppShell.tsx", "utf8");
if (!shell.includes("/os/templates"))
	throw new Error("Templates navigation missing");
