...
async function main() {
	...
	for (const service of services) {
		...
		const endpointOk =
			service.protocol === "http" ? await checkHttp(service.url) : true;
		const ok = running && portOpen && endpointOk;
		failed ||= !ok;
		console.log(
			`${ok ? "OK " : "ERR"} ${service.name.padEnd(10)} state=${row?.State ?? "missing"} port=${portOpen ? "open" : "closed"}${service.protocol === "http" ? ` http=${endpointOk ? "ok" : "fail"}` : ""}`,
		);
	}

	const bucket = process.env.S3_BUCKET ?? "oryon-local";
	const bucketOk = await checkHttp(
		`http://127.0.0.1:9000/${bucket}/`,
		[200, 403],
	);
	failed ||= !bucketOk;
	console.log(
		`${bucketOk ? "OK " : "ERR"} s3-bucket  bucket=${bucket} reachable=${bucketOk ? "yes" : "no"}`,
	);

	if (failed) {
		console.error(
			"Local infrastructure is not ready. Inspect with: pnpm infra:logs",
		);
		process.exit(1);
	}
