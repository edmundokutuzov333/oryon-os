...
for (const path of required) if (!existsSync(path)) throw new Error(`Missing Phase 13 file: ${path}`);
const agentRuntime = readFileSync("packages/ai/src/agent-runtime.ts", "utf8");
const workflowEngine = readFileSync("apps/worker/src/workflow-engine.ts", "utf8");
for (const token of ["requiresCheckpoint", "assertBudget", "rollbackAgentRun", "executeAgentTool"]) if (!agentRuntime.includes(token)) throw new Error(`Agent runtime invariant missing: ${token}`);
for (const token of ["evaluateWorkflowCondition", "executeAgentRun", "executeAgentTool"]) if (!workflowEngine.includes(token)) throw new Error(`Workflow runtime invariant missing: ${token}`);
