...
export function resolveModel(
	...
	if (preferred && (!policy || policy.allowedModels.includes(preferred)))
		return preferred;
	if (policy && policy.allowedModels.length > 0)
		return policy.allowedModels[0] ?? policy.fallbackModel;
...
export function gatewayConfig(): AiGatewayConfig | null {
	...
	return {
		...
		model,
		embeddingModel:
			process.env.ORYON_AI_EMBEDDING_MODEL ?? "text-embedding-3-small",
...
export async function createEmbedding(
	...
	const response = await postJson<Response>(
		`${config.baseUrl}/embeddings`,
		config.apiKey,
		{ model: config.embeddingModel, input: text },
	);
			...
			messages: [
				{
					role: "system",
					content:
						"És o assistente empresarial da OryonOS. Responde apenas com base nas fontes fornecidas. Cita pelo menos uma fonte usando exatamente [source-id]. Não inventes factos nem cites fontes ausentes.",
				},
				{
					role: "user",
