...
const LEVEL_ACTIONS: Record<
	...
	readonly PermissionEvaluation["decisions"][number]["action"][]
> = {
	VIEW: ["read"],
...
function permissionStringMatches(
	permission: string,
	resourceType: string,
	...
	return (
		normalized === action ||
		normalized === `${resourceType.toLowerCase()}:*` ||
		normalized === `${resourceType.toLowerCase()}:${action}`
	);
}
...
function roleAllows(
	...
	for (const role of roles) {
		...
		if (
			...
			return {
				allowed: true,
				matchedBy: `role:${role.scopeType.toLowerCase()}`,
			};
	}
...
function grantMatches(
	...
	return (
		...
		(grant.teamId !== null && subject.teamIds.includes(grant.teamId)) ||
		(grant.externalEmail !== null && grant.externalEmail === subject.email)
	);
}
...
function grantAllows(
	...
	for (const grant of grants) {
		...
		if (action === "use_external" && grant.externalEmail !== null)
			return { allowed: true, matchedBy: "grant:external_email" };
		if (
			action === "use_ai" &&
			(grant.level === "MANAGE" || grant.level === "OWNER")
		)
			return { allowed: true, matchedBy: `grant:${grant.level.toLowerCase()}` };
		if ((LEVEL_ACTIONS[grant.level] ?? []).includes(action))
			return { allowed: true, matchedBy: `grant:${grant.level.toLowerCase()}` };
	}
	return { allowed: false, matchedBy: null as string | null };
}
...
function baseDecision(
	...
	if (context.classification?.blocksAi && action === "use_ai")
		return {
			allowed: false,
			effect: "deny",
			action,
			reason: "classification_blocks_ai",
			matchedBy: `classification:${context.classification.key}`,
		};
	if (context.classification?.blocksDownload && action === "export")
		return {
			allowed: false,
			effect: "deny",
			action,
	...
	if (
		...
		return {
			...
			effect: "allow",
			action,
			reason: "channel_membership",
			matchedBy: "channel_membership",
		};
	if (
		resource.type === "meeting" &&
		context.subject.meetingIds.includes(resource.id) &&
		action === "read"
	)
		return {
			allowed: true,
			effect: "allow",
			action,
			reason: "meeting_membership",
			matchedBy: "meeting_membership",
		};
	if (
		resource.type === "notification" &&
		resource.ownerId === context.subject.id &&
...
function fieldAccess(
	...
	const result: Record<string, "visible" | "masked" | "hidden"> =
		Object.fromEntries(fields.map((field) => [field, "visible"]));
	for (const grant of grants) {
		if (!grantMatches(grant, resource, subject, now)) continue;
		for (const field of grant.fieldMask)
			if (field in result) result[field] = "masked";
	}
	return result;
}
...
export function evaluatePermissions(
	...
	return {
		...
		permissions: {
			...
			use_ai: permissions.use_ai,
			use_external: permissions.use_external,
			view_as: permissions.view_as,
		},
		decisions,
		exposure: {
			external: {
				allowed: input.external && external.allowed,
				reason: input.external ? external.reason : "not_requested",
			},
			ai: {
				allowed: input.ai && ai.allowed,
				reason: input.ai ? ai.reason : "not_requested",
			},
			export: {
