...
const LEVEL_ACTIONS: Record<PermissionLevel, readonly PermissionAction[]> = {
	VIEW: ["read"],
...
function scopeMatches(binding: PermissionRoleBinding, resource: PermissionResource, subject: PermissionSubject): boolean {
	switch (binding.scopeType) {
		...
		case "TEAM":
			return binding.scopeId !== null && subject.teamIds.includes(binding.scopeId);
		default:
			return false;
	}
}
...
function baseDecision(context: PermissionPolicyContext, resource: PermissionResource, action: PermissionAction, now: Date): PermissionDecision {
	...
	if (context.classification?.blocksAi && action === "use_ai") {
		return { allowed: false, effect: "deny", action, reason: "classification_blocks_ai", matchedBy: `classification:${context.classification.key}` };
	}
	if (context.classification?.blocksDownload && action === "export") {
		return { allowed: false, effect: "deny", action, reason: "classification_blocks_download", matchedBy: `classification:${context.classification.key}` };
	}
	...
	if (action === "read" && context.subject.type === "MEMBER" && resource.workspaceId !== null) {
		return { allowed: true, effect: "allow", action, reason: "member_workspace_baseline", matchedBy: "membership" };
	}
