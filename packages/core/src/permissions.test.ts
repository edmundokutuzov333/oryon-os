...
const subject = {
	...
	teamIds: ["team_1"],
	channelIds: ["chn_1"],
	meetingIds: ["meeting_1"],
};
const base: PermissionPolicyContext = {
	orgId: "org_1",
	subject,
	roles: [],
	grants: [],
...
describe("permission engine", () => {
		...
		const decision = can(
			{
				...
				roles: [
					{
						permissions: ["work_object:update"],
						scopeType: "TEAM",
						scopeId: "team_1",
						expiresAt: null,
					},
	...
	it("supports explicit external grants and field masks", () => {
		...
		const result = evaluatePermissions(
			context,
			{ resource, external: true, ai: false, fields: ["title", "moneyAmount"] },
			now,
		);
		...
		expect(
			maskFields(
				{ title: "Visible", moneyAmount: "100" },
				result.exposure.fieldAccess,
			),
		).toEqual({ title: "Visible", moneyAmount: "••••••" });
	});

	it("uses channel membership inside can", () => {
		const channel: PermissionResource = {
			orgId: "org_1",
			type: "channel",
			id: "chn_1",
			workspaceId: null,
		...
		expect(
			can(
				{ ...base, subject: { ...subject, channelIds: [] } },
				channel,
				"read",
				now,
			).allowed,
	...
	it("uses meeting membership inside can", () => {
		const meeting: PermissionResource = {
			orgId: "org_1",
			type: "meeting",
			id: "meeting_1",
			workspaceId: null,
		...
		expect(
			can(
				{ ...base, subject: { ...subject, meetingIds: [] } },
				meeting,
				"read",
				now,
			).allowed,
	...
	it("rejects cross-organization resources", () => {
		expect(
			can(base, { ...resource, orgId: "org_2" }, "read", now).allowed,
		).toBe(false);
	});
});
