...
function adjacency(
	entries: readonly [string, readonly { type: string; id: string }[]][],
): ReadonlyMap<string, readonly { type: string; id: string }[]> {
...
describe("work graph domain", () => {
	it("detects a protected cycle", () => {
		...
		expect(() => assertNoProtectedCycle(c, a, "BLOCKS", graph)).toThrowError(
			GraphDomainError,
		);
	});
	...
	it("rejects self edges before cycle traversal", () => {
		const graph = adjacency([]);
		expect(() => assertNoProtectedCycle(a, a, "PARENT_OF", graph)).toThrow(
			/cannot point to itself/,
		);
	});

	it("resolves out, in and both traversal directions deterministically", () => {
		expect(neighboursForDirection("out", "RELATES_TO", a, b)).toEqual([
			{ node: b, relation: "RELATES_TO", edgeDirection: "out" },
		]);
		expect(neighboursForDirection("in", "RELATES_TO", a, b)).toEqual([
			{ node: a, relation: "RELATES_TO", edgeDirection: "in" },
		]);
