...
export function validateStatusModel(
	input: ObjectTypeDefContract["statusModel"],
): ObjectTypeDefContract["statusModel"] {
	const model = ObjectStatusModelSchema.parse(input);
	const stateKeys = new Set<string>();
	for (const state of model.states) {
		if (stateKeys.has(state.key))
			throw new WorkObjectDomainError(
				"INVALID_STATUS",
				`Duplicate status: ${state.key}`,
			);
		stateKeys.add(state.key);
	}
	if (!stateKeys.has(model.initial))
		throw new WorkObjectDomainError(
			"INVALID_STATUS",
			`Initial status does not exist: ${model.initial}`,
		);
	for (const transition of model.transitions) {
		if (!stateKeys.has(transition.from) || !stateKeys.has(transition.to))
			throw new WorkObjectDomainError(
				"INVALID_TRANSITION",
				`Transition references an unknown status: ${transition.from} -> ${transition.to}`,
			);
	}
	return model;
}
...
function validFieldValue(definition: ObjectFieldDef, value: unknown): boolean {
	...
	throw new WorkObjectDomainError(
		"INVALID_CUSTOM_FIELDS",
		`Unsupported custom field type: ${definition.type}`,
	);
}

export function validateCustomFields(
	config: ObjectTypeDefContract["schema"],
	customFields: Record<string, unknown>,
): Record<string, unknown> {
	const definitionKeys = new Set<string>();
	for (const field of config.fields) {
		if (definitionKeys.has(field.key))
			throw new WorkObjectDomainError(
				"INVALID_CUSTOM_FIELDS",
				`Duplicate custom field: ${field.key}`,
			);
	...
	for (const field of config.fields) {
		...
		if (value === undefined) {
			if (field.required)
				throw new WorkObjectDomainError(
					"INVALID_CUSTOM_FIELDS",
					`Required custom field missing: ${field.key}`,
				);
			continue;
		}
		if (!validFieldValue(field, value))
			throw new WorkObjectDomainError(
				"INVALID_CUSTOM_FIELDS",
				`Invalid custom field: ${field.key}`,
			);
...
export function transitionStatus(
	modelInput: ObjectTypeDefContract["statusModel"],
	...
	if (!current || !target)
		throw new WorkObjectDomainError(
			"INVALID_STATUS",
			`Unknown status: ${currentStatus} -> ${input.status}`,
		);
	if (
		current.key !== target.key &&
		!model.transitions.some(
			(transition) =>
				transition.from === current.key && transition.to === target.key,
		)
	)
		throw new WorkObjectDomainError(
			"INVALID_TRANSITION",
			`Transition not allowed: ${current.key} -> ${target.key}`,
		);
	return { status: target.key, statusCategory: target.category };
}

export function validateWorkObjectDates(
...
export function validateMoney(
	...
	if (amount === undefined || amount === null) {
		if (currency)
			throw new WorkObjectDomainError(
				"INVALID_MONEY",
				"moneyCurrency requires moneyAmount",
			);
		return;
	}
	if (
		...
		throw new WorkObjectDomainError(
			"INVALID_MONEY",
			"moneyAmount and moneyCurrency are invalid",
		);
}
...
export function validateParent(
	...
	if (parentObjectId && id === parentObjectId)
		throw new WorkObjectDomainError(
			"INVALID_PARENT",
			"A work object cannot be its own parent",
		);
}

export function prepareWorkObjectCreate(
	input: WorkObjectCreateInput,
	typeDef: ObjectTypeDefContract,
): WorkObjectCreateInput & { status: string } {
	validateStatusModel(typeDef.statusModel);
	const status = input.status ?? typeDef.statusModel.initial;
	if (!typeDef.statusModel.states.some((candidate) => candidate.key === status))
		throw new WorkObjectDomainError(
			"INVALID_STATUS",
			`Unknown initial status: ${status}`,
		);
...
export function validateWorkObjectUpdate(
	...
	validateMoney(input.moneyAmount, input.moneyCurrency);
	if (input.status)
		transitionStatus(typeDef.statusModel, currentStatus, {
			status: input.status,
		});
}
...
export function generateHumanId(id: string, prefix: string): string {
	...
	if (suffix.length < 6)
		throw new WorkObjectDomainError(
			"INVALID_STATUS",
			"Object id is too short for a human id",
		);
