...
export const PROTECTED_CYCLE_RELATIONS: readonly GraphRelation[] = [
	"BLOCKS",
	"PARENT_OF",
];
...
export function validateGraphSelfEdge(
	...
	if (graphNodeKey(from) === graphNodeKey(to))
...
export function cycleWouldExist(
	...
	while (queue.length > 0) {
		...
		for (const neighbour of adjacency.get(current) ?? []) {
			if (!visited.has(graphNodeKey(neighbour)))
				queue.push(graphNodeKey(neighbour));
		}
	}
	return false;
}
...
export function assertNoProtectedCycle(
	...
	if (cycleWouldExist(from, to, relation, adjacency))
		throw new GraphDomainError(
			"CYCLE_DETECTED",
			`Cycle detected for relation ${relation}`,
		);
}

export function neighboursForDirection(
	...
	if (direction === "out")
		return [{ node: to, relation, edgeDirection: "out" }];
	if (direction === "in")
		return [{ node: from, relation, edgeDirection: "in" }];
	return [
