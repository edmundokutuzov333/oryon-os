...
describe("work object domain", () => {
	it("validates custom fields against the type definition", () => {
		expect(
			validateCustomFields(typeDef.schema, { effort: 3, stage: "a" }),
		).toEqual({ effort: 3, stage: "a" });
		expect(() =>
			validateCustomFields(typeDef.schema, { effort: 3, unknown: true }),
		).toThrow(/Unknown custom field/);
		expect(() => validateCustomFields(typeDef.schema, { stage: "a" })).toThrow(
	...
	it("enforces status transitions", () => {
		expect(
			transitionStatus(typeDef.statusModel, "todo", { status: "doing" }),
		).toMatchObject({ status: "doing", statusCategory: "IN_PROGRESS" });
		expect(() =>
			transitionStatus(typeDef.statusModel, "todo", { status: "done" }),
