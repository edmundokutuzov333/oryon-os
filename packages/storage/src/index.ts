...
export async function createDownloadUrl(
	...
	return getSignedUrl(
		client,
		new GetObjectCommand({ Bucket: required("S3_BUCKET"), Key: storageKey }),
		{ expiresIn },
	);
}
export async function headObject(
	...
	const output = await client.send(
		new HeadObjectCommand({ Bucket: required("S3_BUCKET"), Key: storageKey }),
	);
	if (output.ContentLength === undefined)
		throw new Error("STORAGE_OBJECT_SIZE_MISSING");
