...
export const WorkObjectStatusCategorySchema = z.enum([
	"BACKLOG",
	"TODO",
	"IN_PROGRESS",
	"BLOCKED",
...
export const WorkObjectPrioritySchema = z.enum([
	"LOWEST",
	"LOW",
	"NORMAL",
	"HIGH",
...
export const ObjectFieldTypeSchema = z.enum([
	"TEXT",
	"NUMBER",
	"BOOLEAN",
	"DATE",
...
export const WorkObjectResponseSchema = z.object({
	...
	assignments: z.array(
		z.object({
			...
			allocation: z.number().int().min(0).max(100).nullable(),
		}),
	),
	placements: z.array(
		z.object({
...
export const WorkObjectUpdateInputSchema =
	WorkObjectCreateInputSchema.partial().omit({ typeKey: true });
...
export const WorkObjectAssignmentCreateInputSchema = z
	.object({
	...
	.superRefine((value, ctx) => {
		if (!value.userId && !value.agentId)
			ctx.addIssue({
				code: "custom",
				message: "Assignment must target a user or agent",
			});
		if (value.userId && value.agentId)
			ctx.addIssue({
				code: "custom",
				message: "Assignment cannot target both user and agent",
			});
	});
...
export type WorkObjectAssignmentCreateInput = z.infer<
	typeof WorkObjectAssignmentCreateInputSchema
>;
export type WorkObjectPlacementCreateInput = z.infer<
	typeof WorkObjectPlacementCreateInputSchema
>;
export type WorkObjectStatusInput = z.infer<typeof WorkObjectStatusInputSchema>;
export type WorkObjectTypeCreateInput = z.infer<
	typeof WorkObjectTypeCreateInputSchema
>;
export type ObjectTypeDefContract = z.infer<typeof ObjectTypeDefSchema>;
export type ObjectFieldDef = z.infer<typeof ObjectFieldDefSchema>;
