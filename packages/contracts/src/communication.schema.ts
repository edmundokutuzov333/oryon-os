...
export const ChannelCreateInputSchema = z.object({
	...
	visibility: z.enum(["PUBLIC", "ORG", "TEAM", "PRIVATE"]).default("ORG"),
	linkedObjectId: z.string().min(1).nullable().optional(),
});
export const DirectChannelCreateInputSchema = z.object({
...
export const MessageCreateInputSchema = z.object({
	...
	mediaFileId: z.string().min(1).nullable().optional(),
});
...
export const MessageConversionInputSchema = z.object({
	typeKey: z.string().min(1).default("task"),
	workspaceId: z.string().min(1).nullable().optional(),
	title: z.string().trim().max(500).optional(),
});
export const MessageConversionResponseSchema = z.object({
	messageId: z.string().min(1),
	workObjectId: z.string().min(1),
...
export type DirectChannelCreateInput = z.infer<
	typeof DirectChannelCreateInputSchema
>;
...
export type MessageConversionInput = z.infer<
	typeof MessageConversionInputSchema
>;
export type MessageConversionResponse = z.infer<
	typeof MessageConversionResponseSchema
>;
