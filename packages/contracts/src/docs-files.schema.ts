...
export const PageSchema = z.object({ id: z.string().min(1), orgId: z.string().min(1), workspaceId: z.string().nullable(), parentPageId: z.string().nullable(), title: z.string().min(1), icon: z.string().nullable(), coverUrl: z.string().nullable(), contentYjsBase64: z.string().base64().nullable(), contentJson: z.unknown().nullable(), contentText: z.string().nullable(), kind: PageKindSchema, ownerId: z.string().nullable(), classification: z.string().nullable(), verifiedAt: z.string().datetime({ offset: true }).nullable(), verifiedBy: z.string().nullable(), reviewEveryDays: z.number().int().positive().nullable(), nextReviewAt: z.string().datetime({ offset: true }).nullable(), publishedSlug: z.string().nullable(), publishedAt: z.string().datetime({ offset: true }).nullable(), indexable: z.boolean(), position: z.string(), updatedAt: z.string().datetime({ offset: true }), createdAt: z.string().datetime({ offset: true }), permissions: PagePermissionSchema });
...
export const FileCompleteInputSchema = z.object({ uploadId: z.string().min(1), fileId: z.string().min(1), storageKey: z.string().min(1), name: z.string().trim().min(1).max(255), mimeType: z.string().min(1).max(200), sizeBytes: z.number().int().positive(), checksumSha256: z.string().regex(/^[a-f0-9]{64}$/), classification: z.string().min(1).nullable().optional(), tags: z.array(z.string().min(1).max(80)).max(100).optional() });
