...
import type {
	WorkObjectCommentCreateInput,
	WorkObjectAttachmentCreateInput,
...
export class WorkExperienceRepository {
	...
	async createComment(
		...
		actorId: string,
		objectId: string,
		input: WorkObjectCommentCreateInput,
	) {
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			await appendDomainEvent(tx, {
				...
				name: "work_object.comment.created",
				payload: {
					objectId,
	...
	async attachFile(
		...
		actorId: string,
		objectId: string,
		input: WorkObjectAttachmentCreateInput,
	) {
		return withOrgContext(this.db, orgId, async (tx) => {
			const object = await tx.workObject.findFirst({
				where: { orgId, id: objectId, deletedAt: null },
			...
			await appendDomainEvent(tx, {
				...
				name: "work_object.attachment.created",
				payload: { objectId, fileId: input.fileId },
			});
			return attachment;
		});
	}
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			const [events, transitions] = await Promise.all([
				tx.domainEvent.findMany({
					where: { orgId, subjectType: "WorkObject", subjectId: objectId },
					orderBy: { createdAt: "asc" },
					take: 300,
				}),
			...
			return [
				...
				...transitions.map((transition) => ({
					...
					name: "work_object.status.changed",
