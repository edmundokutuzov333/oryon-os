...
export class WorkExperienceRepository {
  ...
  async listHistory(orgId: string, objectId: string) {
    return withOrgContext(this.db, orgId, async (tx) => {
      ...
      return [
        ...events.map((event) => ({ id: event.id, kind: "DOMAIN_EVENT" as const, name: event.name, actorId: event.actorId, fromStatus: null, toStatus: null, comment: null, occurredAt: event.occurredAt })),
        ...transitions.map((transition) => ({ id: transition.id, kind: "STATUS_TRANSITION" as const, name: "work_object.status.changed", actorId: transition.actorId, fromStatus: transition.fromStatus, toStatus: transition.toStatus, comment: transition.comment, occurredAt: transition.createdAt })),
