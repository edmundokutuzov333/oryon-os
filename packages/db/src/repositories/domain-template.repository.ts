...
function nextSettings(
	settings: unknown,
	templates: InstalledTemplateMap,
...
function toSummary(
	template: DomainTemplateManifest,
	state?: InstalledTemplateState,
): DomainTemplateSummary {
	return {
		key: template.key,
...
export class DomainTemplateRepository {
	...
	async list(orgId: string): Promise<DomainTemplateSummary[]> {
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			return ["crm", "support", "product_engineering"]
				...
				.filter(
					(template): template is DomainTemplateManifest =>
						template !== undefined,
				)
	...
	async find(
		_orgId: string,
		key: string,
	): Promise<DomainTemplateManifest | undefined> {
	...
	async install(
		...
		actorId: string,
		key: string,
	): Promise<DomainTemplateInstallResponse> {
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			const existing = await tx.objectTypeDef.findMany({
				where: {
					orgId,
					key: { in: template.objectTypes.map((type) => type.key) },
				},
			});
			...
			for (const type of template.objectTypes) {
				...
				if (currentType) {
					if (
						!current?.typeDefIds.includes(currentType.id) &&
						!currentType.isSystem
					)
						throw new Error("DOMAIN_TEMPLATE_TYPE_CONFLICT");
					...
					await appendDomainEvent(tx, {
						...
						subjectId: currentType.id,
						name: "domain_template.type.updated",
						payload: {
					...
					await appendDomainEvent(tx, {
						...
						subjectType: "ObjectTypeDef",
						subjectId: row.id,
						name: "domain_template.type.created",
						payload: {
							template: template.key,
							version: template.version,
							key: type.key,
			...
			await tx.organization.update({
				where: { id: orgId },
				data: {
					settings: inputJson(nextSettings(organization.settings, states)),
				},
			});
			await appendDomainEvent(tx, {
				orgId,
				...
				subjectId: orgId,
				name: "domain_template.installed",
				payload: {
					template: template.key,
					version: template.version,
					typeDefIds,
			...
			return {
				...
				version: template.version,
				status: "INSTALLED",
				objectTypeIds: typeDefIds,
				createdCount,
				updatedCount,
				reactivated: current?.status === "INACTIVE",
			};
		});
	}
	...
	async deactivate(orgId: string, actorId: string, key: string) {
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			await appendDomainEvent(tx, {
				...
				subjectType: "Organization",
				subjectId: orgId,
				name: "domain_template.deactivated",
				payload: {
					template: template.key,
					version: current.version,
					typeDefIds: current.typeDefIds,
				},
			});
