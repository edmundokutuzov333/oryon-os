...
export class CommunicationEnhancedRepository extends CommunicationRepository {
    ...
    return withOrgContext(this.database, orgId, async (tx) => {
      ...
      for (const membership of memberships) {
        const [memberCount, unreadCount] = await Promise.all([
          tx.channelMember.count({ where: { orgId, channelId: membership.channelId } }),
          tx.message.count({
            where: {
              orgId,
        ...
        result.push({ channel: membership.channel, memberCount, unreadCount, lastReadAt: membership.lastReadAt });
      }
  ...
  async markChannelRead(orgId: string, userId: string, channelId: string) {
    return withOrgContext(this.database, orgId, async (tx) => {
      ...
      await tx.channelMember.update({ where: { id: membership.id }, data: { lastReadAt } });
      return { channelId, lastReadAt };
    });
  }

  async listPeople(orgId: string, query: string | undefined, limit: number) {
    return withOrgContext(this.database, orgId, (tx) => tx.user.findMany({
      where: {
        orgId,
        status: "ACTIVE",
        deletedAt: null,
        ...(query ? { OR: [{ name: { contains: query, mode: "insensitive" } }, { displayName: { contains: query, mode: "insensitive" } }, { email: { contains: query, mode: "insensitive" } }] } : {}),
      },
      select: { id: true, name: true, displayName: true, avatarUrl: true, presence: true, email: true },
