...
import type {
	ObjectTypeDefContract,
	WorkObjectAssignmentCreateInput,
	WorkObjectCreateInput,
	WorkObjectPlacementCreateInput,
	WorkObjectStatusInput,
	WorkObjectUpdateInput,
	WorkObjectTypeCreateInput,
...
function typeDefContract(row: {
	...
	return {
		...
		schema: row.schema as ObjectTypeDefContract["schema"],
		statusModel: row.statusModel as ObjectTypeDefContract["statusModel"],
	};
}
...
export class WorkObjectRepository {
	...
	async list(input: WorkObjectListInput) {
		...
		return withOrgContext(this.db, input.orgId, async (tx) => {
			...
			return tx.workObject.findMany({
				where,
				include: { assignments: true, placements: true },
	...
	async findById(orgId: string, id: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.workObject.findFirst({
				where: { orgId, id, deletedAt: null },
				include: { assignments: true, placements: true, typeDef: true },
			}),
		);
	}
	...
	async listTypeDefs(orgId: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.objectTypeDef.findMany({ where: { orgId }, orderBy: { name: "asc" } }),
		);
	}
	...
	async createTypeDef(
		orgId: string,
		actorId: string,
		input: WorkObjectTypeCreateInput,
	): Promise<{ id: string }> {
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			await appendDomainEvent(tx, {
				...
				actorId,
				actorType: "MEMBER",
				subjectType: "ObjectTypeDef",
				subjectId: row.id,
				name: "work_object.type.created",
	...
	async create(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			if (prepared.parentObjectId) {
				const parent = await tx.workObject.findFirst({
					where: { orgId, id: prepared.parentObjectId, deletedAt: null },
					select: { id: true },
				});
			...
			const state = typeDef.statusModel.states.find(
			...
			const object = await tx.workObject.create({
				data: {
					...
					humanId: generateHumanId(id, typeRow.idPrefix),
					...
					parentObjectId: prepared.parentObjectId ?? null,
					startAt: prepared.startAt ? new Date(prepared.startAt) : null,
					dueAt: prepared.dueAt ? new Date(prepared.dueAt) : null,
					progress: prepared.progress ?? 0,
					moneyAmount: prepared.moneyAmount,
					moneyCurrency: prepared.moneyCurrency,
					probability: prepared.probability,
					secondaryDate: prepared.secondaryDate
			...
			await appendDomainEvent(tx, {
				...
				subjectId: object.id,
				name: "work_object.created",
				payload: {
					typeKey: object.typeKey,
					humanId: object.humanId,
					title: object.title,
					primaryWorkspaceId: object.workspaceId,
				},
			});
			return object.id;
		});
	...
	async update(
		...
		actorId: string,
		id: string,
		input: WorkObjectUpdateInput,
	) {
		return withOrgContext(this.db, orgId, async (tx) => {
			const current = await tx.workObject.findFirst({
				where: { orgId, id, deletedAt: null },
				include: { typeDef: true },
			});
			if (!current) throw new Error("NOT_FOUND");
			const typeDef = typeDefContract(current.typeDef);
			const mergedCustomFields = input.customFields
				? {
						...(current.customFields as Record<string, unknown>),
						...input.customFields,
					}
				: undefined;
			const validationInput = mergedCustomFields
				? { ...input, customFields: mergedCustomFields }
				: input;
			validateWorkObjectUpdate(validationInput, typeDef, current.status);
			validateWorkObjectDates(
				input.startAt ?? current.startAt?.toISOString(),
				input.dueAt ?? current.dueAt?.toISOString(),
			);
			const moneyAmount =
				input.moneyAmount !== undefined
					? input.moneyAmount
					: (current.moneyAmount?.toString() ?? null);
			const moneyCurrency =
				input.moneyCurrency !== undefined
					? input.moneyCurrency
			...
			if (input.parentObjectId) {
				const parent = await tx.workObject.findFirst({
					where: { orgId, id: input.parentObjectId, deletedAt: null },
					select: { id: true },
				});
				if (!parent) throw new Error("PARENT_NOT_FOUND");
			}
			...
			let transition: {
				status: string;
				statusCategory: ObjectTypeDefContract["statusModel"]["states"][number]["category"];
			} | null = null;
			if (input.status) {
				transition = transitionStatus(typeDef.statusModel, current.status, {
					status: input.status,
				});
				statusCategory = transition.statusCategory;
				completedAt = transition.statusCategory === "DONE" ? new Date() : null;
			}
			const data: Prisma.WorkObjectUncheckedUpdateInput = {
				statusCategory,
				completedAt,
			};
			...
			if (input.parentObjectId !== undefined)
				data.parentObjectId = input.parentObjectId;
			...
			if (input.moneyCurrency !== undefined)
				data.moneyCurrency = input.moneyCurrency;
			if (input.probability !== undefined) data.probability = input.probability;
			if (input.secondaryDate !== undefined)
				data.secondaryDate = input.secondaryDate
					? new Date(input.secondaryDate)
					: null;
			...
			if (mergedCustomFields !== undefined)
				data.customFields = JSON.parse(
					JSON.stringify(mergedCustomFields),
				) as Prisma.InputJsonValue;
			const updated = await tx.workObject.update({
				where: { id },
				data,
				include: { assignments: true, placements: true, typeDef: true },
			});
			if (
				...
				await tx.objectPlacement.updateMany({
					where: { orgId, objectId: id, containerType: "WORKSPACE" },
					data: { isPrimary: false },
				});
				if (input.workspaceId)
					await tx.objectPlacement.upsert({
						where: {
							objectId_containerType_containerId: {
								objectId: id,
								containerType: "WORKSPACE",
								containerId: input.workspaceId,
							},
						},
						create: {
							orgId,
			...
			await appendDomainEvent(tx, {
				...
				subjectType: "WorkObject",
				subjectId: id,
				name: "work_object.updated",
				payload: { changed: Object.keys(data), status: updated.status },
			});
			return updated;
		});
	}
	...
	async assign(
		...
		actorId: string,
		objectId: string,
		input: WorkObjectAssignmentCreateInput,
	): Promise<{ id: string }> {
		return withOrgContext(this.db, orgId, async (tx) => {
			const object = await tx.workObject.findFirst({
				where: { orgId, id: objectId, deletedAt: null },
				select: { id: true },
			});
			...
			if (input.userId) {
				...
				const agent = await tx.agent.findFirst({
					where: { orgId, id: input.agentId, deletedAt: null },
					select: { id: true },
				});
			...
			await appendDomainEvent(tx, {
				...
				name: "work_object.assignment.created",
				payload: {
					objectId,
					userId: input.userId ?? null,
					agentId: input.agentId ?? null,
	...
	async removeAssignment(
		...
		await withOrgContext(this.db, orgId, async (tx) => {
			const assignment = await tx.assignment.findFirst({
				where: { id: assignmentId, orgId, objectId },
			});
			...
			await appendDomainEvent(tx, {
				...
				name: "work_object.assignment.deleted",
				payload: { objectId },
			});
		});
	}
	...
	async place(
		...
		actorId: string,
		objectId: string,
		input: WorkObjectPlacementCreateInput,
	): Promise<{ id: string }> {
		return withOrgContext(this.db, orgId, async (tx) => {
			const object = await tx.workObject.findFirst({
				where: { orgId, id: objectId, deletedAt: null },
			...
			if (input.isPrimary)
				await tx.objectPlacement.updateMany({
					where: { orgId, objectId },
					data: { isPrimary: false },
				});
			const placement = await tx.objectPlacement.upsert({
				where: {
					objectId_containerType_containerId: {
						objectId,
			...
			await appendDomainEvent(tx, {
				...
				actorId,
				actorType: "MEMBER",
				subjectType: "ObjectPlacement",
				subjectId: placement.id,
				name: "work_object.placement.upserted",
				payload: {
					objectId,
					containerType: input.containerType,
					containerId: input.containerId,
	...
	async status(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			const transition = transitionStatus(
				typeDefContract(current.typeDef).statusModel,
				current.status,
				input,
			);
			const completedAt =
				transition.statusCategory === "DONE" ? new Date() : null;
			const updated = await tx.workObject.update({
				where: { id },
				data: {
			...
			if (transition.status !== current.status)
				await tx.statusTransition.create({
					data: {
						orgId,
						objectId: id,
