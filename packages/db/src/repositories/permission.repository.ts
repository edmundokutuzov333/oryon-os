...
export class PermissionRepository {
	...
	async getSnapshot(orgId: string, userId: string, resourceType: string, resourceId: string, classificationKey?: string | null): Promise<PermissionSnapshot> {
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			const [grants, classification] = await Promise.all([
				...
				classificationKey
					? tx.classificationLabel.findFirst({ where: { orgId, key: classificationKey } })
	...
	async listExternalExposure(orgId: string): Promise<ExternalExposureRow[]> {
		return withOrgContext(this.db, orgId, async (tx) => {
			const rows = await tx.accessGrant.findMany({
				...
				select: { resourceType: true, resourceId: true, fieldMask: true, expiresAt: true },
			});
			...
			for (const row of rows) {
				...
				if (current) {
					...
					current.fieldMasked = current.fieldMasked || row.fieldMask.length > 0;
				} else {
					grouped.set(key, {
	...
	async createRole(orgId: string, input: { key: string; name: string; description?: string; permissions: string[] }): Promise<{ id: string }> {
		return withOrgContext(this.db, orgId, async (tx) => {
			const role = await tx.role.create({ data: { orgId, key: input.key, name: input.name, description: input.description, permissions: input.permissions, isSystem: false } });
			return { id: role.id };
		});
	}
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			await tx.domainEvent.create({
				data: {
					...
					payload: { roleId: input.roleId, principalId: input.principalId, scopeType: input.scopeType, scopeId: input.scopeId ?? null },
				},
			});
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			await tx.domainEvent.create({
				data: {
					...
					subjectId: grant.id,
					payload: { resourceType: input.resourceType, resourceId: input.resourceId, level: input.level },
				},
			});
			return { id: grant.id };
		});
