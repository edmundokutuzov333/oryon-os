	...
	async list(orgId: string, pageId: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.attachment.findMany({
				where: { orgId, objectId: null, targetType: "page", targetId: pageId },
				include: { file: true },
	...
	async attach(orgId: string, actorId: string, pageId: string, fileId: string) {
		return withOrgContext(this.db, orgId, async (tx) => {
			const page = await tx.page.findFirst({
				where: { orgId, id: pageId, deletedAt: null },
				select: { id: true },
			});
			if (!page) throw new Error("NOT_FOUND");
			const file = await tx.fileAsset.findFirst({
				where: { orgId, id: fileId, deletedAt: null },
				select: { id: true },
			});
			if (!file) throw new Error("FILE_NOT_FOUND");
			const existing = await tx.attachment.findFirst({
				where: { orgId, fileId, targetType: "page", targetId: pageId },
			});
			...
			await appendDomainEvent(tx, {
				...
				name: "page.attachment.created",
				payload: { pageId, fileId },
			});
			return attachment;
		});
	}
