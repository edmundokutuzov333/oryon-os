...
export class AutomationRepository {
	...
	async listOrganizations(): Promise<string[]> {
		const rows = await this.db.$queryRaw<
			Array<{ id: string }>
		>`SELECT id FROM public.oryon_worker_organization_ids()`;
		return rows.map((row) => row.id);
	}
	async list(orgId: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.workflow.findMany({ where: { orgId }, orderBy: { name: "asc" } }),
		);
	}
	async findById(orgId: string, id: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.workflow.findFirst({ where: { id, orgId } }),
		);
	}
	async create(
		orgId: string,
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			const row = await tx.workflow.create({
				data: {
					orgId,
					key: input.key,
					name: input.name,
					description: input.description ?? null,
					triggerJson: input.trigger as Prisma.InputJsonValue,
			...
			await appendDomainEvent(tx, {
				...
				payload: { key: row.key, name: row.name } as Prisma.InputJsonValue,
			});
			return { id: row.id };
		});
	}
	async setState(
		...
		await withOrgContext(this.db, orgId, async (tx) => {
			...
			await appendDomainEvent(tx, {
				...
				payload: { from: current.state, to: state } as Prisma.InputJsonValue,
			});
		});
	}
	async createRun(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			const workflow = await tx.workflow.findFirst({
				where: { id: workflowId, orgId, state: "ACTIVE" },
			});
			...
			await appendDomainEvent(tx, {
				...
				actorType: actorId ? "MEMBER" : "SERVICE_ACCOUNT",
	...
	async findRunByTriggerEvent(
		...
		return withOrgContext(this.db, orgId, (tx) =>
			tx.workflowRun.findFirst({
				where: { orgId, workflowId, triggerEventId },
				select: { id: true },
			}),
		);
	}
	async updateRun(
		...
		await withOrgContext(this.db, orgId, async (tx) => {
			const current = await tx.workflowRun.findFirst({
				where: { id, orgId },
				select: { workflowId: true, workflow: { select: { createdBy: true } } },
			});
			if (!current) throw new Error("NOT_FOUND");
			const data: Prisma.WorkflowRunUpdateInput = {};
			if (patch.state !== undefined) data.state = patch.state;
			if (patch.stepLog !== undefined)
				data.stepLogJson = patch.stepLog as Prisma.InputJsonValue;
			if (patch.error !== undefined)
				data.errorJson =
					patch.error === null
						? Prisma.JsonNull
						: (patch.error as Prisma.InputJsonValue);
			if (patch.waitingOnApprovalId !== undefined)
				data.waitingOnApprovalId = patch.waitingOnApprovalId;
			if (patch.finishedAt !== undefined) data.finishedAt = patch.finishedAt;
			await tx.workflowRun.update({ where: { id }, data });
			await appendDomainEvent(tx, {
				orgId,
				actorId: actorId ?? current.workflow.createdBy,
				actorType: "MEMBER",
				subjectType: "WorkflowRun",
				subjectId: id,
				name: "workflow.run.updated",
				payload: {
	...
	async listActiveEventWorkflows(
		orgId: string,
	): Promise<Array<{ id: string; triggerJson: Prisma.JsonValue }>> {
		return withOrgContext(this.db, orgId, async (tx) =>
			(
				...
				(row) =>
					WorkflowTriggerSchema.safeParse(row.triggerJson).success &&
					(row.triggerJson as { kind: string }).kind === "EVENT",
			),
		);
	}
