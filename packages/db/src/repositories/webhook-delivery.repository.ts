...
export class WebhookDeliveryRepository {
	...
	async listWebhookTargets(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			return hooks.filter(
				(hook) =>
					hook.active &&
					hook.events.some((event) => event === "*" || event === eventName),
			);
		});
	}
	...
	async getWebhook(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			if (!org) return null;
			const hook = (parseSettings(org.settings).webhooks ?? []).find(
	...
	async createDelivery(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			const existing = await tx.webhookDelivery.findUnique({
				where: { webhookId_eventId: { webhookId, eventId } },
				select: { id: true },
			});
			if (existing) return { id: existing.id, created: false };
			const row = await tx.webhookDelivery.create({
				data: { orgId, webhookId, eventId, status: "PENDING" },
				select: { id: true },
			});
