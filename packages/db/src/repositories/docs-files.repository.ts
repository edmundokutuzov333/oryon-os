...
export class DocsFilesRepository {
	...
	async findPage(orgId: string, id: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.page.findFirst({ where: { orgId, id, deletedAt: null } }),
		);
	}
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			if (input.parentPageId) {
				const parent = await tx.page.findFirst({
					where: { orgId, id: input.parentPageId, deletedAt: null },
					select: { id: true },
				});
			...
			const position = await tx.page.count({
				where: {
					orgId,
			...
			await appendDomainEvent(tx, {
				...
				subjectId: page.id,
				name: "page.created",
				payload: { title: page.title, kind: page.kind },
			});
			return page;
		});
	}
	async updatePage(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			if (
				...
				input.parentPageId !== current.parentPageId
			) {
				...
				for (let depth = 0; depth < 100 && cursor; depth += 1) {
					...
					const next = await tx.page.findFirst({
						where: { orgId, id: cursor, deletedAt: null },
			...
			const data = {
				...
				...(input.contentYjsBase64 === undefined
					? {}
					: {
							contentYjs:
								input.contentYjsBase64 === null
									? null
									: Buffer.from(input.contentYjsBase64, "base64"),
						}),
				...(input.classification === undefined
					? {}
					: { classification: input.classification }),
				...(input.reviewEveryDays === undefined
					? {}
					: {
							reviewEveryDays: input.reviewEveryDays,
							nextReviewAt:
								input.reviewEveryDays === null
									? null
									: new Date(Date.now() + input.reviewEveryDays * 86400000),
						}),
			...
			const page = await tx.page.update({
				where: { id },
				data: data as Prisma.PageUncheckedUpdateInput,
			});
			const hasContent =
				input.contentJson !== undefined ||
				input.contentText !== undefined ||
			...
			if (hasContent) {
				const last = await tx.pageVersion.findFirst({
					where: { orgId, pageId: id },
					orderBy: { version: "desc" },
					select: { version: true },
				});
				const snapshot = input.contentYjsBase64
					? Buffer.from(input.contentYjsBase64, "base64")
					: Buffer.from(
							JSON.stringify(
								input.contentJson ??
									page.contentJson ?? { type: "doc", content: [] },
							),
							"utf8",
						);
			...
			await appendDomainEvent(tx, {
				...
				subjectId: id,
				name: "page.updated",
				payload: { changed: Object.keys(data), contentVersioned: hasContent },
			});
			return page;
		});
	}
	...
	async publishPage(
		...
		return withOrgContext(this.db, orgId, async (tx) => {
			...
			const next =
				slug === null
					? { publishedSlug: null, publishedAt: null }
					: { publishedSlug: slug, publishedAt: new Date() };
			...
			await appendDomainEvent(tx, {
				...
				subjectType: "Page",
				subjectId: id,
				name: slug === null ? "page.unpublished" : "page.published",
				payload: { slug },
			});
			return updated;
		});
	...
	async createFile(
		orgId: string,
		actorId: string,
		input: FileCompleteInput & { storageKey: string },
	) {
		return withOrgContext(this.db, orgId, async (tx) => {
			const existing = await tx.fileAsset.findFirst({
				where: { orgId, checksumSha256: input.checksumSha256, version: 1 },
			});
	...
	async findFile(orgId: string, id: string) {
		return withOrgContext(this.db, orgId, (tx) =>
			tx.fileAsset.findFirst({ where: { orgId, id, deletedAt: null } }),
		);
	}
