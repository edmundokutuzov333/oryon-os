...
export function fuseHybrid(
	...
	for (const [index, hit] of lexical.entries()) {
		...
		if (!existing.matchedBy.includes("lexical"))
			existing.matchedBy.push("lexical");
...
export class TypesenseSearchClient {
	...
	async ensureCollection(): Promise<void> {
		try {
			...
			if (!(error instanceof Error) || !error.message.includes("TYPESENSE_404"))
				throw error;
		}
	...
	async upsertMany(documents: readonly SearchDocument[]): Promise<number> {
		...
		const response = await fetch(
			`${this.url}/collections/${SEARCH_COLLECTION}/documents/import?action=upsert`,
			{
				method: "POST",
				headers: {
		...
		if (failed.length > 0)
			throw new Error(`TYPESENSE_IMPORT_FAILED_${failed.length}`);
		return lines.length;
	}
	...
	async lexical(
		...
		const params = new URLSearchParams({
			q: normalize(query),
			query_by: "title,contentText",
			query_by_weights: "4,1",
			prioritize_exact_match: "true",
			per_page: String(Math.min(limit, 50)),
			page: "1",
			filter_by: `orgId:=${orgId}${type ? ` && type:=${type}` : ""}`,
		});
		type Response = {
			hits?: Array<{
				document: SearchDocument;
				text_match?: number;
				highlight?: { contentText?: string; title?: string };
			}>;
		};
		const response = await this.request<Response>(
			`/collections/${SEARCH_COLLECTION}/documents/search?${params.toString()}`,
		);
		return (response.hits ?? []).map((hit, index) => ({
			...
			snippet:
				hit.highlight?.contentText ??
				hit.highlight?.title ??
				hit.document.contentText.slice(0, 240),
			textRank: hit.text_match ?? index + 1,
		}));
	}
}
